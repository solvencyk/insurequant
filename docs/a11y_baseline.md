...
Rows 7–9 and 11–13 below were fixed in the same session, verified with `scripts/a11y_contrast_check.py` before editing (not eyeballed). Row 10 needed no change (already compliant) and row 14 remains open (content-authoring task, different category — see below).
...
| 12 | `index.html` treemap swatch + `colorForRatio()` (the live per-cell color function) — red↔green diverging scale | Endpoint contrast drops to ~1.1–1.3:1 under simulation — classic red-green diverging problem | Below-threshold side (`hue=0` red) → `hue=215` blue, in both `colorForRatio` branches (kics + basicCapital) and the `.swatch` legend gradient. Above-threshold stays green. Blue↔green endpoints verified at ΔRGB 63–122 under simulation (mobile list bars inherit the fix automatically — same function). Legend caption text "빨강 <130%..." → "파랑 <130%...". **`.bubble-swatch` CSS class is dead code** (defined, never inserted into markup — the bubble chart colors dots by sector via `SECTOR_COLOR`, not this gradient) — left as-is, not a live issue, noted here so it isn't mistaken for still-broken |
