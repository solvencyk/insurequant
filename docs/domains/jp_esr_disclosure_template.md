...
추출해 `extracted_sample_values.json` 에 `tokiomarine_nichido` / `mitsui_sumitomo` / `sompo_japan` 을 추가했다(ESR 층은 3사 전부 not_yet — 2026-10-31).
**결과: 3사 모두 profit 31/31 applicable 전항목, history 10/10, 게이트 실패 0**(informational P13 만 예정대로 실패), `census_match` 3/3.

**페이지 매핑(1-idx, fitz `doc[p-1]`).**

| 회사 | 損益計算書 | 保険引受利益 明細 | 比率표(3개년, 合算率 직접) | 5개년표 | 재보험 다리 | 出再先·格付 | 責任準備金の内訳 | 회계기준 근거 |
|---|---|---|---|---|---|---|---|---|
| Tokio Marine & Nichido Fire (`tmnf_2026_full.pdf`) | p102 (2열 2024/2025, 比較増減 없음) | p90 | p91 「正味損害率、正味事業費率及びその合算率」 | p88 | p89~91 | p92 | p118 「責任準備金の残高内訳」 | p102(責任準備金戻入額)+p109(会社法第436条) → B 티어 jgaap |
| Mitsui Sumitomo Insurance (`msi_2026_full.pdf`) | p109 (2열, **세로쓰기 라벨 + 내역행 괄호값**) | p100 「(11) 保険引受利益の推移」 | p99 「(8) …合算率」 | **p31**(会社の概況 절 — 직전 티켓의 "5개년 단일표 없음" 은 오판, 損害率 2자리 소수) | p94~97 | p55 | p123 「③ 責任準備金の種目別残高の内訳」(2024/2025 별도표) | p107(会計方針)·p109·p114 → B 티어 jgaap |
| Sompo Japan Insurance (`sompojapan_2026_full.pdf`) | p136 (2열) | p122 「17 保険引受利益」 뒤 区分표 | p120 「10 …およびその合算率」 | p116 「1.最近5事業年度に係る主要な財務指標」 | p117~119 | p121~122 | p151 「3. 責任準備金の内訳」 | p136+p142 → B 티어 jgaap |

**추출값(百万円, cur=FY2025 / prev=FY2024).**

| 항목 | TMNF | MSI | Sompo Japan |
|---|---|---|---|
| 経常収益 / 経常費用 | 3,764,871 / 2,834,061 (3,948,277 / 2,787,730) | 2,492,658 / 1,832,388 (2,453,546 / 1,877,519) | 2,856,236 / 2,479,171 (2,870,540 / 2,555,960) |
| 経常利益 | 930,809 (1,160,546) | 660,270 (576,026) | 377,064 (314,579) |
| 保険引受利益 | 47,741 (96,949) | 115,762 (54,601) | 48,251 (△51,908) |
| 資産運用損益(P&L 資産運用収益−費用, `pl_stmt` 대체) | 938,672 (1,116,791) | 567,850 (540,617) | 360,675 (389,283) |
| 当期純利益 | 731,125 (949,719) | 459,965 (459,900) | 295,649 (256,982) |
| 正味収入保険料 / 正味支払保険金 | 2,596,396 / 1,454,585 | 1,754,431 / 970,472 | 2,311,577 / 1,339,037 |
| 元受 / 受再 / 出再 보험료 | 2,862,377(含む積立) / 322,829 / 540,476 | 2,017,006(除く積立) / 192,437 / 455,012 | 2,662,630(含む積立) / 179,447 / 466,747 |
| 元受 / 受再 / 回収 보험금 | 1,420,151 / 264,344 / 229,910 | 998,449 / 164,499 / 192,475 | 1,381,751 / 178,590 / 221,304 |
| 損害率 / 事業費率 / 合算率 | 61.6 / 31.4 / 93.0 (63.1 / 31.6 / 94.7) | 62.8 / 30.4 / 93.2 (66.7 / 32.0 / 98.7) | 63.8 / 33.3 / 97.0 (66.1 / 33.9 / 100.0) |
| 異常危険準備金 합계 / 火災 · 普通責任準備金 합계 | 1,072,011 / 393,148 · 2,054,269 | 590,001 / 127,651 · 1,475,745 | 693,262 / 288,055 · 1,927,281 |
| 出再先数 / 上位5社 / A格以上 | 154社 / 61.7% / 99.3% | 209社 / 40.0% / 99.6%(2026年4月末 기준 표) | 102社 / 47.5% / 98.9% |
| 회계기준 | jgaap / IFRS17 false | jgaap / false | jgaap / false |

**회사별 편차와 처리(코드 스위치는 `COMPANIES` 항목의 키 이름).**

- **재보험 다리 표 구조 3종**(`BRIDGE_SPEC`): TMNF 는 受再/支払 · 受再/回収 을 **한 표에 두 열**로 찍는다(合計 행 6개 금액 → 짝수/홀수 열로 분리). Sompo 는
  「出再正味保険料」「出再正味保険金」이라는 **다른 라벨**(스키마 labels_ja 에 추가). MSI 는 ①~④ 원문자 번호 — `norm()` 이 NFKC 라 "②"→"2" 로
  들어오므로 정규식은 숫자로 쓴다. 합계행은 금액(정수)만 골라 마지막 2개 = prev/cur(`_amount_tokens`) — 構成比·増減率은 소수점이 있어 자동 제외.
- **元受正味保険料 가 "含む収入積立保険料" 표뿐인 회사(TMNF·Sompo)**: 재보험 항등식이 収入積立保険料 만큼 안 닫힌다(TMNF 48,334 / Sompo 63,753).
  MSI 는 「除く収入積立保険料」 표가 따로 있어 그 값을 쓴다. TMNF/Sompo 는 공시값 그대로 저장하고 `bridge_gross_incl_deposit=True` →
  損益計算書 「収入積立保険料」 행을 `profit.adjustments.deposit_premium_in_gross` 로 뽑아 **P14 에서 차감**(3사 P14/P15 8/8 통과). 추정 아님, 원문 두 행의 산수.
- **P07(経常利益 브리지) 식 수정**: 保険引受利益 안의 その他収支(自賠責 法人税相当額)는 経常利益에 없다 → 식에 `− その他収支` 추가. au(null)·Meiji(−1/−2)는
  tol 3 안이라 티 안 났고 TMNF 는 −3,152 라 이 항 없이는 실패, 넣으면 **정확히 0 차이**. P11 tol 1→3(4항 절사, Sompo FY2025 48,253 vs 48,251).
- **資産運用損益**: 3사 모두 積立保険料等運用益 ≠ 0(TMNF 26,259) 이라 §9-6 Meiji 와 같은 이중계상 → `pl_investment_override="pl_stmt"` 를 라벨 앵커
  P&L 에도 적용(`資産運用収益`/`資産運用費用` 행 grab). 利回り 표는 아예 안 읽는다(profit_pages 에 inv 없음).
- **회계기준 IFRS 판정 범위**: 3사 본편 뒷부분에 **그룹 연결 IFRS17 재무제표**가 같이 실려 있어 문서 전체 검색이면 単体 손익이 ifrs 로 오판된다 →
  IFRS/IFRS第17号 검색을 `basis` 페이지로 한정(`ifrs_scope_basis_pages`, BRIDGE_SPEC 회사 기본 on). 근거문에 "連結 section is IFRS/IFRS17 at group level" 명시.
- **責任準備金の内訳**: 3사 다 種目 × (普通責任/異常危険/危険/払戻/契約者配当/合計) 인데 셀이 붙어 나온다(TMNF "－1,109,472"·"2,074,170 1,027,463",
  Sompo "自動車損害賠償責任 417,739 |") → 라벨 사이 텍스트에서 숫자를 정규식으로 12개(2024|2025) 또는 6개 뽑아 FY2025 쪽 사용(`RESERVE_SPEC`).
  합계 = 5개년표 責任準備金残高와 3사 전부 일치(3,996,578 / 2,684,258 / 3,362,832). 自賠責 異常危険準備金 은 대시 → null 정상.
- **出再先·格付 표**: TMNF 는 셀마다 "(－)"(제3분야 병기) 가 붙고 BBB格以上 열이 **누적**(A格以上 포함) — A格以上 + その他 만 버킷 합(=100). MSI 격付 표는
  "2026年4月末" 기준(出再先数·上位5社는 2025年度). Sompo 는 연도가 행이 아니라 열(2024/2025 값이 세로로 두 줄).
- **Sompo Japan 폰트 함정(§10-7 후속)**: 業績データ 절(p116~)은 서브셋 MS-PGothic/YuGothic 에 ToUnicode 가 없어 fitz 가 **글리프 id 를 코드포인트로**
  내보낸다. 숫자만이 아니라 한자·가나 전부다. 실측 규칙: ASCII = cp+16044, 히라가나 = cp+5776, 가타카나 = cp+5774, 전각 기호 몇 개는 고정 id, 한자는
  **MS Gothic 글리프 순서와 정확히 일치**(`fitz.Font(fontfile=msgothic.ttc).has_glyph(cp)` 가 gid 를 돌려주므로 BMP 전체를 뒤집어 gid→cp 표를 만든다).
  `GidDoc`/`GidPage` 가 `get_text("dict")` 의 span 폰트명으로 깨진 폰트만 골라 복원(정상 Iwata CID 폰트 span 은 그대로). 이 PC 의 `C:/Windows/Fonts/msgothic.ttc`
  에 의존 — 없으면 경고 후 미복원(값 NOT_FOUND). 다른 그룹사가 같은 조판이면 `gid_decode=True` 만 켜면 된다.
...
> FY2021~FY2025 5개년 전체를 담는 별도 층을 신설했다. 기계 스키마 = `esr_disclosure_schema.json` `layer:"history"` 13항목(값 10 + 생보 id 3개는
> 정의만). 값은 `{"FY2021": v, ..., "FY2025": v}` 사업연도 키 dict(공시 표의 열 순서 = 오래된→최신, 회사와 무관하게 동일). 추출·검산 =
> `extract_esr_template_samples.py::extract_history / run_history_checks`. 발행 블록(`jp/jesr_detail.json` `companies[].history`)은
> `build_jesr_detail_json.py::build_history_block`이 같은 값을 `fiscal_years`+`series`(연도 배열, `[v,...]`) 형태로 다시 편다.

### 10-1. 표 위치

**「主要な経営指標等の推移」**(직역: 주요 경영지표 등의 추이) — 告示 양식이 아니라 会社法・보험업법상 매 결산서류에 관행적으로 들어가는 5개년
요약표. 두 회사 모두 손익계산서 바로 **앞쪽**(業績データ/현황 책자의 시작부)에 있고, `profit` 층이 이미 P10 교차검산용으로 읽던
`profit_pages["summary5"]` 페이지와 **동일 페이지**다(같은 표를 다시 읽을 뿐, PDF 를 새로 열지 않는다).

| 회사 | 문서 | 페이지 | 항목수(직접 공시) | 비고 |
|---|---|---|---|---|
| au Non-Life | `au_nonlife_disclo_260730_4of5.pdf`(業績データ편, 4of5 분책 — **지표 표는 본편이 아니라 이 분책 안에 있었다**, 본편 필요 없음) | p2 | 8행: 正味収入保険料·経常収益·経常利益·保険引受利益·当期純利益·正味損害率·正味事業費率·利息及び配当金収入 + 자산/부채 7행(純資産額·総資産額 등) + SMR/ESR 1행 | 정미손해율·정미사업비율이 **표에 직접 있음**(5개년 전부) |
| Meiji Yasuda Non-Life | `meijiyasuda_nonlife_20260729_main.pdf`(본편, 60p — §9-6 에서 확보한 그 파일) | p9(격付評価 박스 바로 아래) | 正味収入保険料(対前期増減率 병기)·経常収益·経常利益·当期純利益·純資産額·総資産額(うち積立勘定)·責任準備金残高·配当性向·従業員数 + SMR/ESR 1행 | **손해율·사업비율이 이 표엔 없다** — `(6)正味損害率、正味事業費率およびその合算率`(p35, 3개년 종목별 표, profit 층이 이미 읽는 표)에서 FY2023~2025 만 백필, FY2021~2022 는 null |

### 10-2. 연도 수·라벨 렌더링

- 두 회사 모두 정확히 5개 사업연도(FY2021~FY2025), 열 순서 오래된→최신 — 공통. `HIST_FISCAL_YEARS = ["FY2021",...,"FY2025"]` 고정 상수(10월
  62사도 이 순서를 그대로 가정하되, 5개년보다 짧은 회사가 나오면 `row()`가 모자란 연도를 뒤에서부터 `None` 패딩한다).
- au 는 이 페이지가 완전히 세로쓰기 라벨(한 글자 한 줄)이라 기존 `merge_vertical()`을 그대로 재사용. Meiji 본편 p9 는 **라벨 렌더링이
  행마다 다르다** — 正味収入保険料/純資産額/総資産額/責任準備金残高는 이미 한 줄, 経常収益/経常利益/当期純利益/資本金/貸付金/有価証券残高/従業員数는
  한 글자씩 세로쓰기, **단체SMR 라벨은 그 중 한 글자(`ー`, 長音符)가 대시 문자와 동일 코드포인트라 `merge_vertical`이 글자 단위 병합을 멈춰버려
  "単体ベ"/"ー"/"スの"/"ソルベンシー・マージン比率" 로 쪼개진다.** 정확한 라벨 앵커 대신 `HIST_SMR_LABEL_RE = r"ソルベンシー.{0,3}マージン比率"`
  느슨한 부분일치로 우회(다른 행엔 영향 없음, au 도 안 깨짐 확인).
- 값과 주석(괄호)이 같은 줄 스트림에 섞여 나오는 행이 둘 있다: Meiji 正味収入保険料(값 뒤에 `（対前期増減率）`, 예: `14,822`→`（0.5%）`) 와
  総資産額(값 뒤에 `（−）`=적립계정 없음). `strip_paren()`+`PAREN_DASH_RE` 로 괄호 안 숫자와 괄호 안 대시를 구분해서: 전자는 항목 자체가
  버려지는 주석이라 `drop_paren=True` 로 통째 스킵, 후자는 전역으로 항상 스킵(항상 노이즈).

### 10-3. 구기준 SMR ↔ 신기준 ESR — 같은 행, 괄호 유무로 분리

두 회사 모두 5개년표에 SMR/ESR 이 **한 행**으로 있고, 신제도(FY2025) 전환 각주가 붙는다:

- au: 주기 5 "단체SMR은 2026년 3월기 결산부터 「경제가치기준」 신제도가 시작됐다. 이 때문에 구제도에 의한 전년도 이전 숫자의 기재를 생략한다."
  → FY2021~2024 는 표에 **대시**로 찍힘(진짜 구기준 실적이 아니라 "생략"), FY2025 만 비괄호 791.7%.
- Meiji: 주기 2 "「단체SMR」의 (　) 안 수치는 구기준에 의한 단체 SMR 이다" → FY2021~2024 가 **괄호로 구기준 실측치**(2,847.6%/2,940.4%/
  2,814.7%/2,642.5%), FY2025 만 비괄호 743.2%(신기준).
- 추출 규칙: **괄호 있는 토큰 → `hist_smr_old_pct`, 괄호 없는 토큰 → `hist_esr_pct`**(공통 파서, 회사별 분기 없음). au 는 구기준 자리가
  전부 대시(→null)라 `hist_smr_old_pct` 가 5개 다 null 로 나오는 게 **정상**(생략이지 결측 아님) — Meiji 는 4개 non-null.
- `hist_esr_pct` 는 두 회사 다 FY2025 한 칸만 채워진다 — 신제도 첫 적용연도라 구조적으로 그렇다(10월 62사도 대부분 같을 것).

### 10-4. 검산 (`run_history_checks`, H01~H02)

- **H01**: `hist_ordinary_profit`/`hist_net_income`/(손보만)`hist_net_premiums_written`/`hist_loss_ratio_pct`/`hist_expense_ratio_pct`/
  `hist_combined_ratio_pct` 의 FY2025 가 `profit` 층 같은 id 의 `cur` 와, FY2024 가 `prev` 와 각각 일치하는지(금액 ±1, 비율 ±0.1) — **두 개의
  서로 다른 표(5개년표 vs 損益計算書/比率표)가 같은 숫자를 찍는지 교차확인**. 결과: au 12/12, Meiji 10/10(합산율 FY2025 는 5개년표에 직접
  없어 3개년 표에서 백필한 값이라 이 교차확인 자체가 백필 소스와의 자기일치 확인이 됨).
- **H02**: 연도별 `合算率 = 損害率 + 事業費率`(±0.1). au 는 5개년표 세 행이 다 직접 공시라 5/5(FY2021~2025), Meiji 는 백필된 FY2023~2025
  3개년만 3/3(FY2021~2022 는 損害率 자체가 없어 검산 대상에서 빠짐, 결측이지 실패가 아니다).
- 결과: **au 17/17, Meiji Yasuda Non-Life 15/15**(둘 다 gate 실패 0). `extract_esr_template_samples.py` exit 0 전체 게이트에 포함(`all_checks`
  에 H01/H02 합류) — 기존 C/A/G/P 체크와 같은 gate 로 묶여 있어 history 가 깨지면 스크립트 exit code 도 1 이 된다.

### 10-5. 회사별 편차 (10월 62사 주의점)

- **손해율/사업비율이 5개년표에 직접 있는 회사(au형)와 없는 회사(Meiji형)가 갈린다.** 없는 회사는 `(6)正味損害率…` 류 3개년표에서
  FY2023~2025 만 백필하고 FY2021~2022 는 null 로 남긴다 — 억지로 다른 표에서 끌어오지 않는다(`profit_pages["ratio"]` 가 없는 회사는 그마저도
  못 채우고 전부 null).
  raw_tokens 에 `"BACKFILL from profit:ratio 3yr table"` 문자열이 남아 이 경로를 탄 회사를 바로 구분할 수 있다.
- **합산율(合算率) 자체가 5개년표에 직접 있는 경우는 거의 없다** — 두 표본 다 없어서 `hist_loss_ratio_pct + hist_expense_ratio_pct` 로 파생하거나
  (au) 백필 소스인 3개년표의 세 번째 열(합산율도 이미 그 표에 직접 있다)을 그대로 쓴다(Meiji). 둘 다 항등식으로 닫히는지 H02 로 검산.
- **생보 id 3개(`hist_core_profit`/`hist_premium_income`/`hist_policy_reserves`)는 이번 라운드 표본이 전부 손보라 라벨 정의만 해뒀다** —
  10월에 생보사를 뽑을 때 `基礎利益`/`保険料等収入`/`責任準備金残高` 라벨로 같은 `row()` 워커가 그대로 동작할 것으로 예상되지만 실측 전이라
  검증 안 됨. 生保 5개년표(예: NN Life p11)는 이번 티켓 범위 밖이라 손대지 않았다(라벨·페이지 구조가 au/Meiji 와 같은지 확인 필요).
- 라벨 렌더링(세로쓰기·대시 혼입·괄호 주석)은 **회사마다 다르다** — 새 회사를 추가할 때 `row()` 를 그대로 믿지 말고 §10-2 처럼 페이지
  원문을 먼저 덤프해서 확인할 것(이번에도 Meiji SMR 라벨의 "ー"=장음부호 문제는 사전에 예측 못 하고 원문 대조로 발견했다).

### 10-6. 중간기(中間期) 공시 — 일본은 법정 공시 주기가 한국과 다르다

일본 보험업법은 **연차(3월말) + 中間期(9월말, 半期)** 결산서류만 법정 의무다. 한국의 분기(1Q/3Q) 같은 **법정 분기 공시가 없다** — 이 5개년표도
매년 3월말 기준 1회만 나온다(9월 中間期에는 요약 손익만 별도 서식으로 나오고, 이 「主要な経営指標等の推移」 표는 없음, 표본 미확인이라 추정).
상장 지주 6사(Tokio Marine·MS&AD·Sompo·T&D·Sony FG·Dai-ichi)는 도쿄증권거래소 適時開示 의무로 **분기 決算短信**(헤드라인 손익·그룹 지표만,
이 정도로 상세한 보험사 개별 5개년표는 없음)을 낸다 — jp 레인이 연 1회(7월 전후) census/history 를 도는 이유이자, "9월 재census" 에서도
history 층은 **갱신되지 않는다**(다음 갱신은 FY2026 결산이 나오는 2027년 7월 전후). census 3축(`docs/domains/claude-agent-jp.md` §4b)과
같은 원문을 재활용하므로 별도 추가 조사는 필요 없다.

### 10-7. 표본 6사 실측 (2026-09-13, 티켓 `inbox/jp/20260913T0230Z__owner__JP_MULTI__nonlife_ratio_availability.md`)

owner 질의: "손해율 5개년이 다른 손보사에도 다 있는지, FY2025 없어도 FY2024 까지는 어딘가 공시돼 있지 않나?" — 대형 4(東京海上日動·損保
ジャパン·三井住友海上·あいおいニッセイ同和) + 중형 2(共栄火災·日新火災)를 표본으로 잡았으나, 이 세션에서 curl 이 라운드 중간에 **전
도메인 차단**(`www.google.com` 포함 전부 `Failed to connect`, PC 시간대별 차단 — `claude-agent-jp.md` §4 의 알려진 제약)으로 바뀌어
6사 중 **3사만 원문 PDF 를 실제로 확보**했다. 미확보 3사는 행을 지우지 않고 사유를 남긴다(census 원칙과 동일).

| 회사 | 확보 | 표 존재 | 페이지(0-idx) | 연도수 | 손해율/사업비율/합산율 라벨 | FY2024 값 | FY2025 값 | ESR/SMR 행 상태 |
|---|---|---|---|---|---|---|---|---|
| Tokio Marine & Nichido Fire (東京海上日動, 단체) | ○ (`tmnf_2026_full.pdf`, TMNF_2026_d.pdf, 292p) | ○ 「主要な経営指標等の推移」p87 | 87(단체 5개년), 127~128(연결, IFRS 3개년+日本基準 5개년 별도표) | 5(FY2021~2025) | 正味損害率・正味事業費率(**合算率 행 없음**, 파생만 가능) | 損害率 63.1% / 事業費率 31.6% | 損害率 61.6% / 事業費率 31.4% (合算率 파생 93.0%) | not_yet — 표 안 셀 자체가 "別時期での開示"(신규 라벨, 단체·연결 공통), 주석: "新制度導入初年度における経過措置の適用により、2026年10月末までに開示します" |
| Mitsui Sumitomo Insurance (三井住友海上, 단체) | ○ (`msi_2026_full.pdf`, a01.pdf, 272p) | △ 5개년 단일표는 못 찾음(목차에 있는 "主要な経営指標等の推移" p132 는 **연결 IFRS 2개년 그룹표**뿐) — 대신 3개년 종목별표 (8)「正味損害率、正味事業費率及びその合算率」p98 에 **合算率 컬럼이 직접 공시** | 96~99 | 3(FY2023~2025, 종목별+合計 행) | 正味損害率・正味事業費率・**合算率**(단독 라벨로 직접 공시, 파생 불필요) | 合計행: 損害率 66.7% / 事業費率 32.0% / 合算率 98.7% | 合計행: 損害率 62.8% / 事業費率 30.4% / 合算率 93.2% | not_yet — 자회사(MS プラスワン少額短期保険) SMR 표 옆 각주(p241 注4): "三井住友海上のソルベンシー・マージン比率（2025年度数値）は、保険業法等に基づき2026年10月末までに開示します"(신규 문구) — 리스크관리 절(p47)에 ESR 목표치 "180％以上を確保" 별도 명시(注: ESR=経済価値ベースのソルベンシー・レシオ) |
| Sompo Japan Insurance (損害保険ジャパン, 단체) | ○ (`sompojapan_2026_full.pdf`, sj_disc2026.pdf, 292p) | ○ but **제목이 다르다** — 「1.最近5事業年度に係る主要な財務指標」p115(신규 표제목 변형, "経営指標" 대신 "財務指標"), 별도 3개년 종목별표 (10)「正味損害率、正味事業費率およびその合算率」p119 | 115(5개년 단일표), 119(3개년 종목별+合算率) | 正味損害率・正味事業費率(5개년 단일표엔 合算率 없음) + **合算率**(3개년 종목별표 合計행에 직접 공시) | 5개년표: 損害率 66.1% / 事業費率 33.9% · 3개년표 合計행: 合算率 100.0% | 5개년표: 損害率 63.8% / 事業費率 33.3% · 3개년표 合計행: 合算率 97.0% | not_yet — 5개년표 SMR/ESR 행이 FY2021~2025 전부 대시(－), 주석5: "2025年度の単体ベースのソルベンシー・マージン比率の開示は2026年10月末の予定です"(신규 문구) |
| Aioi Nissay Dowa (あいおいニッセイ同和, 단체) | **미확보** | — | — | — | — | — | — | — (`corporate/policy/disclo_policy.html`·`/ir/` 페이지 확인했으나 PDF 링크가 JS 동적 로드로 추정 — href 없음. `ms-ad-hd.com` IR 라이브러리 페이지는 이 PC 에서 403/커넥션 거부. WebSearch 로도 직접 PDF URL 미발견) |
| Kyoei Fire & Marine (共栄火災, 단체) | **미확보** | — | — | — | — | — | — | — (`www.kyoeikasai.co.jp` curl 2회 연결거부 + WebFetch 인증서 오류 — 이 세션 중 도메인이 아예 접속불가로 전환됨) |
| Nisshin Fire & Marine (日新火災, 단체) | **미확보** | — | — | — | — | — | — | — (WebSearch 로 페이지 구조는 확인: `/ir/disclosure.html` 에 「日新火災の現状 2026」`/ir/pdf/disclosure2026.pdf` 6.4MB 존재 확인, 그러나 curl 연결거부로 실제 PDF 를 못 열어봐서 손해율 실측은 미실시 — URL 만 census `disclosure_url` 후보로 남김) |

**결론 (owner 질의에 대한 답).** 표본 3사(대형, 확보분) 전부 **손해율·사업비율 5개년(또는 최소 FY2024~2025) 시계열은 이미 공시돼 있다** —
owner 가정이 맞다. 다만 세 회사가 서로 다른 방식으로 담고 있어 "한 자리에서 5개년 손해율+합산율을 바로 읽을 수 있는 회사"는 없다:
- **표 제목이 통일돼 있지 않다**: au/Meiji/TMNF 는 「主要な経営指標等の推移」, Sompo Japan 은 「最近5事業年度に係る主要な財務指標」(신규 변형,
  `esr_disclosure_schema.json` 은 아직 미반영 — 값 파싱 자체는 `row()` 라벨매칭이라 영향 없지만 표 탐색을 제목 문자열로 하면 놓친다).
- **合算率(합산율)이 5개년 단일표에 직접 있는 회사는 이번 표본 3사 중 0사** — au/Meiji 와 동일 패턴(§10-5 재확인). 대신 MSI·Sompo Japan
  둘 다 **별도 3개년 종목별표에는 合算率 이 라벨로 직접 있다**(파생 불필요, `hist_combined_ratio_pct` labels_ja 에 `"合算率"` 단독 추가).
  이 3개년표는 종목(火災/海上/傷害/自動車/自賠責/その他)별 행 + 合計 행 구조라 5개년표보다 항목이 많다 — 10월 62사 확장 시 이 표가 있으면
  손해율·사업비율·합산율 3종을 **한 표에서 동시에** 뽑을 수 있어 우선순위가 높다.
- **ESR(신기준)은 표본 3사 전부 미공표(not_yet), 예외 없음** — 대형사도 10월 말 기한을 그대로 쓴다. 문구는 회사마다 또 달랐다:
  TMNF 는 표 셀 안에 "別時期での開示"(다른 시기에 개시), Sompo Japan 은 표 셀은 대시(－)+각주 산문, MSI 는 표 자체가 안 보이고
  자회사 SMR 표 옆 각주 산문 — **셋 다 새 문구**라 `esr_disclosure_schema.json` `esr_status` labels_ja 에 3종 추가(별시기での開示 /
  …2026年10月末までに開示します / …の予定です).
- **폰트 인코딩 함정 1건 발견**: Sompo Japan PDF 의 숫자/기호(`0-9`,`.`,`%`,공백 등)가 PyMuPDF 로 추출하면 U+3EDC 대역으로 전부
  **+16044(0x3EAC) 오프셋**된 PUA 유사 코드포인트로 나온다(임베디드 폰트 ToUnicode CMap 이상 — TMNF/MSI 는 정상). 한자·가나는 정상
  추출되고 숫자만 깨지므로 못 보고 지나칠 수 있다. 이 회사(그리고 같은 폰트를 쓸 다른 그룹사)를 다룰 때는 `ord(ch)-16044` 가
  32~126 범위면 ASCII 로 치환하는 디코더를 거칠 것 — `docling`/`fitz` 어느 쪽도 자동 보정하지 않는다.
- **미확보 3사(Aioi·Kyoei·Nisshin)는 원인이 다르다** — Aioi 는 사이트 구조상 PDF 직링크를 못 찾은 경우(원문 자체가 없다는 뜻 아님),
  Kyoei/Nisshin 은 이 세션 중 curl 대상 도메인이 접속 자체가 막힌 네트워크 제약(Nisshin 은 WebSearch 로 정확한 URL은 확인함:
  `https://www.nisshinfire.co.jp/ir/pdf/disclosure2026.pdf`). 10월 재census 라운드에서 curl 이 풀렸을 때 그대로 재시도할 것 —
  회사가 공시를 안 했다는 근거는 없다.

### 10-8. 대형 손보 3사 5개년표 (2026-09-13, 티켓 `inbox/jp/20260913T0330Z__owner__JP_MULTI__big3_partial_detail.md`)

- **TMNF p88** 「主要な経営指標等の推移」: 금액 행마다 `(対前期増減(△)率)` 부라벨 + 값 뒤 `(x.x%)` 성장률 괄호 → `hist_skip=[対前期増減]`,
  `hist_drop_paren_all=True`. 損害率/事業費率 5개년 직접(57.5→61.6 / 31.9→31.4), 合算率 파생. SMR 행은 대시 4개 + "別時期での開示" → 전부 null 정상.
  5개년표의 인수이익 라벨은 「保険引受損益」(利益 아님).
- **MSI p31**: 라벨과 「（対前期増減率）」이 세로쓰기로 한 줄에 합쳐지고("正味収入保険料(対前期増減率)"), 성장률 셀이 "(\x08" / "1.27% )" / "- )(\x08"
  세 조각(괄호 뒤 백스페이스 글리프)으로 나온다 → `hist_label_overrides`(접두 매칭) + skip 정규식 3종. **損害率/事業費率이 소수 2자리**(62.85/30.44) 라
  H01 은 tol 0.1 + 1e-9(3개년표 62.8 과 0.05 차), 파생 合算率 93.3 vs 3개년표 直接 93.2 도 0.1 안. SMR 행은 「新基準|旧基準|(新,旧)×5년」 쌍 구조
  (`hist_smr_layout="new_old_pairs"`): 旧基準 FY2021~24 = 722.5/684.3/691.1/706.3, FY2025 는 "(注1)"(미공표) → `hist_smr_old_pct` 4개 non-null,
  `hist_esr_pct` 전부 null. 같은 값으로 `article_axes.smr_old_basis_fy2024_pct=706.3` 을 덮는다(`smr_from_history` — 기존 범용 정규식은 이 쌍 구조에서
  FY2022 값을 집는다).
- **Sompo p116** 「最近5事業年度に係る主要な財務指標」: 부라벨 「（対前年度増減率）」(TMNF 와 한 글자 다름) → `hist_skip=[対前年度増減率]`. 글리프 복원(§9-8) 후
  기존 `row()` 그대로. 合算率은 5개년표에 없어 파생(FY2025 97.1) — 3개년표 직접값 97.0 과 0.1 차(profit 층은 직접값). SMR 행 전부 대시 → null.
- 3사 모두 5개년표 == 損益計算書/比率표 교차(H01) 통과: TMNF 12/12, MSI 12/12, Sompo 12/12 + H02 5/5 씩. P10(summary5 xref) 은 이 3사에선
