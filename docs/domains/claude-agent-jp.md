...
**한정어(qualifier) 목록도 이 절이 정본이다**(2026-09-13 신설 · **2026-09-14 전수 등재**, UH-23). 같은 문서에
**같은 라벨로 여러 값**이 있고 그중 일부는 조건이 붙은 조정치다 — 그걸 헤드라인으로 잘못 고른 것이
かんぽ生命 220% 사고였다(정답 181%). 기계본은 `J-ESR/check_esr_in_source.py::ADJUSTED_QUALIFIERS` 이고,
**아래 표와 집합으로 일치**해야 한다. 늘리거나 줄이려면 **표를 먼저 고치고** 15사에 다시 돌려 정상사 발화 0 을
확인한다. 한쪽만 고치면 검증기가 정본과 다른 목록으로 검증한다(K-ICS 상관행렬 재타이핑 금지와 같은 병).

> 2026-09-13 에는 이 절이 관측된 3종만 산문으로 적고 있었고 코드는 10종을 들고 있었다 — 게다가 이 절이
> 「추측으로 넣지 말 것」 으로 지목한 `調整後` 를 코드가 실제로 들고 있었다(실측: 코드 10종 중 §3 에 문자열로라도
> 있던 것은 4종). 그래서 UH-23 은 절반만 닫혀 있었고, 2026-09-14 에 표 등재 + 양방향 대조 테스트로 닫았다.

| 한정어 | 15사 관측 | 왜 목록에 있나 |
|---|---|---|
| `除いた場合` | 2 | かんぽ p35 「大量解約リスクを除いた場合のESRは220%」 — 사고 그 문장 |
| `除いたESR` | 0 | 같은 구문의 이형 |
| `除くESR` | 0 | 같은 구문의 이형 |
| `除外した` | 0 | 같은 구문의 이형 |
| `調整後` | 0 | PM seed. 「調整後ESR」 은 공시 관행 표기라 남긴다 |
| `適正水準` | 4 | かんぽ. **빼면 안 된다** — 빼면 p18 「ESR適正水準 150~220%」 가 한정어 없는 조각이 되어 220 이 빠져나간다(실측) |
| `ターゲットレンジ` | 1 | 富国 「ESRをターゲットレンジ内(230%~270%)に維持」 |
| `目標レンジ` | 0 | 같은 뜻의 이형 |
| `参考値` | 0 | PM seed 「参考」 를 좁힌 형태 — 맨 「参考」 는 각주 기호와 충돌한다 |
| `試算値` | 0 | 이형 |

관측 0 인 7종은 **추측 어휘가 아니라 관측된 구문의 이형·공시 관행 표기**다. 맨 `を除く`·`レンジ`·`目安`·`参考`
처럼 넓은 형태로는 **일부러 안 넣었다**(각주 기호·목차와 충돌). 관측도 이형 근거도 없는 새 어휘를 넣는 것은 금지.

**넣으면 안 되는 것(정의 표지)**: 아래는 "어느 ESR 이냐" 지 "조정했다" 가 아니다. 문면상 구분이 안 되지만 성격이
다르다 — 한정어 목록에 넣으면 정상 3사(日本生命·住友生命·朝日生命)가 거짓 발화한다(실측). 같은 대조 테스트가
이 넷이 코드 목록에 **없음**을 강제한다.

| 정의 표지 | 어디에 붙어 있나 (그 회사의 **정답** 헤드라인) |
|---|---|
| `ベース` | 住友生命 「経済価値ベースのソルベンシー比率」 — 라벨 자체다 |
| `内部管理` | 朝日生命 「ESR(グループ)(内部管理ベース)は258.9%」 |
| `規制` | 日本生命 「規制ESR…連結:195%」 |
| `速報値` | 富国生命의 다른 값(210%)에 붙어 있다 |

판정식은 「라벨동반 산문 조각 ≥1(0이면 기권) × 전부 한정어 × 같은 문서에 한정어 없는 대안값 존재」 3곱이다.
대조 테스트는 `tests/test_jp_source_gate.py::test_adjusted_qualifiers_match_the_domain_doc`
(문서에 없는 한정어를 코드가 들면 FAIL · 표에만 있고 코드에 없어도 FAIL · 정의 표지가 코드에 섞이면 FAIL).
