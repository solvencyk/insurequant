...
**한정어(qualifier) 목록도 이 절이 정본이다**(2026-09-13 신설, UH-23). 같은 문서에 **같은 라벨로 여러 값**이 있고
그중 일부는 조건이 붙은 조정치다 — 그걸 헤드라인으로 잘못 고른 것이 かんぽ生命 220% 사고였다(정답 181%).
15사 라벨동반 조각 전수에서 **실제로 관측된** 한정어는 셋뿐이다: `除いた場合` · `適正水準` · `ターゲットレンジ`.
(`を除く`·`レンジ`·`目安`·`調整後`·`参考` 는 관측 0 — 추측으로 넣지 말 것.)
**넣으면 안 되는 것**: 「規制ESR」·「内部管理ベース」 같은 **정의 표지**는 한정어가 아니다. 문면상 구분이 안 되지만
성격이 다르다 — 정의 표지를 한정어 목록에 넣으면 정상 3사(日本生命·住友生命·朝日生命)가 거짓 발화한다(실측).
기계본은 `J-ESR/check_esr_in_source.py::ADJUSTED_QUALIFIERS`, 판정식은 「라벨동반 산문 조각 ≥1(0이면 기권) ×
전부 한정어 × 같은 문서에 한정어 없는 대안값 존재」 3곱이다.
