# CLAUDE.md 이력 — 2026-09-12 축약 이전 전문 (읽기 지연)

> 2026-09-12 에 `CLAUDE.md` 를 룰만 남기고 ~100줄로 줄였다(owner 지시: 22k 자로 비대). 룰이 생긴 배경 서사·사고 기록은
> 여기 **한 글자도 고치지 않고** 옮겨 두었다. changelog 처럼 특정 룰의 근거가 필요할 때만 연다. 세션 시작 시 읽지 않는다.
> 상단 룰의 정본은 항상 현재 `CLAUDE.md`; 여기 내용과 어긋나면 현재 파일이 이긴다.
