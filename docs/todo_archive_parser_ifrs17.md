...
> `TODO_parser_ifrs17.md` 의 Status 이력 블록을 2026-09-11 에 **한 글자도 고치지 않고** 옮긴 것(최신순). 세션 시작 시 읽지 않는다 — `docs/changelog_*.md` 처럼 특정 과거 결정의 배경이 필요할 때만 연다. 활성 TODO 의 Status 에서 밀려난 항목은 이 줄 바로 아래에 그대로 잘라 붙인다(최신이 위).
...
>
> 발주: `status_report.py --fast` 축5가 2025.3Q~2026.2Q 4개 분기에서 항목5를 39사 중
> 8~10사 hole로 보고. 4개 분기 hole 35셀(회사 10개)을 전수 판정 — **(a)raw 있는데 파싱
> 안 됨 0건 · (b)원문에 항목 없음 35건 · (c)raw 자체 없음 0건.** 상세:
> `inbox/_resolved/20260901T1320Z__parser__MULTI__ifrs17_bs_item5_surrender_reserve_gap_
> census.md`.
>
> **8사(KR0050 하나손해·KR0051 신한이지·KR0070 에이비엘생명·KR0083 푸본현대생명·KR0100
> 처브라이프생명·KR0150 서울보증·KR1010 교보라이프플래닛·KR1098 카카오페이손보)는 16분기
> 전체에서 항목5 실값이 단 한 번도 없다** — `scripts/reserve_extract/` 패키지(2026-08 세션
> 작성)가 이미 회사별 핸들러로 raw 전수검색해 부재를 확정해 둔 상태였고, 이번 세션이 8사
> 전원의 최신 필링에서 정확한 개념어 `해약환급금준비금`으로 재검색해 0건 재확인(에이비엘은
> 느슨한 검색어 `해약환급금`이 상품명 `(해약환급금 일부지급형)` 등 11건 오탐을 내는 함정이
> 있어 개념어를 정확히 좁혀 확인). **KR0069(삼성생명) 2025.1Q~3Q**도 raw 3개 필링을 직접
> 읽어 "해약환급금준비금" 문자열이 정확히 2회(감독규정 설명 문단뿐, 값 표 없음)뿐임을
> 확인 — 분기·반기 필링엔 준비금 노트가 아예 없다(사업보고서 전용, 2025.4Q부터 처음 등장).
> **KR0072(케이디비생명)는 2026-08-21에 이미 등재 완료라 무변경.**
>
> **조치 = `data/_gold/statutory_reserve_legit.json`의 `disclosed_none`에 신규 8개사
> 엔트리 + 삼성생명 기존 엔트리에 2025.1~3Q 3분기 추가**(raw 인용 + `reserve_extract`
> 핸들러 교차참조 포함). 마스터(`IFRS17_BS.json`)·빌더·핸들러는 무변경 — `build_ifrs17_bs.py`
> 미실행. `git status`로 변경파일이 gold JSON 1개뿐임을 확인.
>
> **게이트 실측**: `validate_statutory_reserves.py` 등재 전 `RED=0 BASELINE=12 ORANGE=49
> SUPPRESSED=64` → 등재 후 `RED=0 BASELINE=12 ORANGE=48 SUPPRESSED=64`(삼성생명 item5의
> R-RSV-9 "첫 공시 이전 3셀 부재" 정확히 1건 소멸, 나머지 불변). 신규 8사는 애초에
> `holders[5]`(항목5 실공시 이력 있는 회사)에 안 잡혀 R-RSV-9 census 대상이 아니었다 —
> 등재의 실익은 감사이력 문서화 + 향후 이 8사 중 하나가 첫 실값을 공시하는 순간
> `holders[5]`에 편입되며 과거 전 분기가 소급 census 되는 것을 미리 막아두는 것
> (삼성생명·케이디비생명에 실제로 있었던 패턴).
>
> **"화면 합계 과소" 정정**: R-RSV-10(업권 앵커) 2026.2Q 실측 `업권 합 61,756,170 vs
> 보도 58,100,000 (+6.3%). 셀결측 0사 / BS부재 0사` — 항목5 실공시 이력 있는 회사는
> 2026.2Q 전원 값이 채워져 있다. 원 발주의 "hole 만큼 과소" 가정은 `status_report.py`의
> naive row-presence 카운트에서 나온 것으로, 이 8~10사는 애초에 업권 합계 기여분이 없는
> (한 번도 공시한 적 없는) 회사들이라 화면 합계는 과소되지 않았다(오히려 보도치 대비
> +6.3%, 별도 이슈·비차단 ORANGE로 기존 처리 중, 이 스코프 밖).
>
> 골든(`tests/test_ifrs17_bs_golden.py`) 대상 무변경(빌더 미실행) — 재실행 불필요.
> git commit/push 안 함(오케스트레이터 일괄 처리). K-ICS·PL_breakdown 레인 파일 무변경.
...
> 둘 다 정정(라이나 4,713.86억 · AIG 318.64억), 나머지 30사 값 불변. 배포 완료.
>
> owner 상시 규칙("화면 그래프는 전부 마스터에")에 대해 **이 패널만 시트가 없었다** —
> 마스터의 `금리민감도`(K-ICS 지급여력비율 금리 ±bp)와 이름이 비슷해 오인돼 온 자리다.
> `CSM_sensitivity.json`(195행) 신설 + xlsx `가정민감도` 시트 + 공개 다운로드 등재.
> 그 규칙은 이제 `test_every_live_fetched_artifact_lands_in_a_master_sheet` 가 강제한다
> (화면 fetch 15개 전부 시트 대응, gap 0).
>
> **미결 1건 발주**: `inbox/parser/20260830T2000Z` — 에이아이에이 항목5 가 2024.1Q~3Q 만
> 빈다. 빌더 재실행 결과 값 diff 0 이라 스냅샷이 아니라 로직 문제이고, owner 수용기준
> 2024.6월말 −6.2% 미달의 대부분이 이 한 건이다.
...
> **2026-08-31 (2) — 2026.2Q 라운드 2회차: 생보 3사 게시 · item5 신규 24칸 등재(owner 승인).**
>
> **탐색 2회차**: 손보 1사(하나손해) 유지, **생보 3사가 게시로 뒤집혔다** — 라이나(KR0074) ·
> BNP카디프(KR0075) · 메트라이프(KR0095). 1회차에 타임아웃이던 한화손해도 정상 관측(미관측 0).
> 생보협회 일괄은 2분기 열이 아직 비어 있어 자사 사이트에서 받아야 했고, 그 경로가 없어
> `scripts/download_disclosure_2026q2_life_sites.py` 신설. 세 사이트가 서로 다른 스택이라
> **회사별 XPath 가 필수**다 — 라이나는 `<button>PDF 다운로드</button>`(//a 로는 0건),
> BNP 는 `a.filedown` 자체가 행 제목, 메트라이프는 `.pdf` 를 담은 앵커.
> `verify_q2_disclosure_content.py` **4/4 ACCEPT**(신선도·2026.6.30 기준·공시 문서형).
>
> **해약환급금준비금 비교(owner: 크게 다르지 않으면 미수정)**
>
> | 회사 | 분기 | 경영공시 | 마스터 | 판정 |
> |---|---|---|---|---|
> | 메트라이프 | 2026.2Q | 2,733,000 | 2,021,231 | **+35.2% → 교체** |
> | 라이나 | 2026.2Q | 4,027,700 | 3,939,468 | +2.2% → 교체(같은 복사 결함) |
> | 라이나·메트라이프 | 2026.1Q | 4,028,100 / 2,313,000 | 동일 | **일치(교차검증)** |
> | BNP카디프 | 2026.2Q | −(미적립) | 0 | 불필요 |
> | 하나손해 | 2026.2Q | 0 | 행 없음 | legit zero(원문 명시) |
>
> 두 교체 건 모두 마스터가 **2025.4Q 값을 소수점까지 복사**하고 있어 2026.2Q 실측이 없었다.
> 같은 표의 `직전 분기` 열이 앞서 고친 2026.1Q 와 정확히 일치해 **48셀 작업이 다른 필링으로
> 독립 검증**됐다. 미교체 셀 중 1% 초과 차이는 9건뿐이고 최대가 분기 업권합계의 **0.53%** —
> owner 기준상 손댈 것 없음.
>
> **신규 24칸 등재(owner 승인)**: 마스터에 셀 자체가 없던 (회사,분기). 대부분 2023.1~3Q 로,
> 그 구간 모집단이 8~14사뿐이라 통째로 빠져 있던 회사들이다(신한라이프 2023.1Q 3.05조 ·
> 한화생명 2023.1~3Q · DB생명 2023.1~3Q · 메트라이프·AIA·IBK연금 등).
> **비교열만 있는 20건과 필링간 불일치 3건은 쓰지 않았다**(DB손해 2023.2Q 는 두 필링이
> 2,714,600 vs 1,917,200 으로 갈린다). 6칸은 그 분기에 다른 17BS 항목이 없는 **단독 행**인데,
> 게이트 코어는 `IFRS17_BS_CORE_ITEMS = (1,2,3,4)` 이고 5·6·7 은 owner 가 optional 로 못박은
> 축이라("코어로 올리지 말 것") census 를 건드리지 않는다 — 게이트로 실측 확인.
>
> 마스터 6,858 → **6,885행**. 전수 diff: 변경·추가 **77셀 전부 항목5** · 오버라이드 값과 불일치 0 ·
> 누락 0. 예별손해 item5 2025.4Q~2026.2Q 3칸이 `None→0` 으로 늘어난 것은 2025.3Q=0 의 빌더 이월.
>
> **업권 합계 — 2023년이 크게 개선됐다**
>
> | 분기 | 전 | 후 | 비고 |
> |---|---|---|---|
> | 2023.1Q | 11.86조 | **18.92조** | |
> | 2023.2Q | 16.33조 | **24.36조** | |
> | 2023.3Q | 20.15조 | **29.34조** | 2023.4Q 31.92조와 자연스럽게 연결(종전 20.15→31.92 계단) |
> | 2026.2Q | 60.96조 (+4.9%) | **61.76조 (+6.3%)** | ⚠ ±5% 이탈 |
>
> **2026.6말 편차가 +4.9% → +6.3% 로 ±5% 를 벗어났고, owner 가 그대로 두라고 결정했다
> (2026-08-31): "raw 데이터에 확실히 있는 숫자면 굳이 기사 숫자에 안 맞춰도 된다. 걍 참고용이었다."**
> → **보도치 앵커(2023년말 32.2조 · 2024.6말 38.5조 · 2026.6말 58.1조)는 참고용 sanity check 이지
> 종결조건이 아니다.** 원문에 있는 값과 앵커가 어긋나면 **원문을 남긴다**. 되돌리지 않았다
> (되돌렸다면 라이나만 +6.1% · 메트라이프만 +5.1% · 둘 다 +4.9%였다).
> 같은 이유로 2024.6말의 -4.2% → +4.4% 부호 전환도 종결로 본다.
>
> **게이트**: `validate_data_contract` RED=0 exit=0(법정준비금 **신규 RED=0**) · 골든 fixture +
> 입력지문 **동시** 갱신 · xlsx `17BS` cherry-pick(요약 행수 6858→6885 자동 보정).

> **2026-08-31 — item5 기중값이 `기적립액`만 담고 있었다. 경영공시 잔액으로 48셀 교체 (owner 지시).**
>
> owner: "기중(1Q~3Q) 차액을 고쳐라. 단 새로 찾은 숫자를 두 눈 크게 뜨고 확인해라. 업계 합산도
> 기사 보도치와 비교해라." 이어서 라이나 2배를 보고 **"의심스럽지 않니?"** 라고 반문 — 그 반문이
> 아래 원인 정정과 농협생명 철회를 만들었다.
>
> **원천**: K-ICS 정기경영공시 PDF 의 `5-3.`(회사에 따라 `7-2.`/`7-3.`)
> **해약환급금준비금 등의 적립** 표. 4행 × 2열(**해당 분기 · 직전 분기**), 단위 억원(×100 = 백만원).
>
> **원인은 '연말값 이월' 이 아니라 '개념 절단' 이다.** 재무제표 주석이 분해를 명시한다 —
> `기적립액 + 적립예정금액 = 잔액`. 라이나생명 7개 분기 전수 대조 결과:
>
> | 분기 | 주석 기적립액 | 주석 예정액 | 주석 잔액 | 5-3 표 | 마스터(기존) |
> |---|---|---|---|---|---|
> | 2024.1Q | 2,251,256 | 2,267,194 | 4,518,450 | 4,518,400 | **2,251,256** |
> | 2024.2Q | 2,251,256 | 2,327,962 | 4,579,218 | 4,579,200 | **2,251,256** |
> | 2024.3Q | 2,251,256 | 2,417,446 | 4,668,702 | 4,668,700 | **2,251,256** |
> | 2025.1Q | 3,720,603 | 0 | 3,771,192 | 3,771,200 | **3,720,603** |
> | 2026.1Q | 3,939,468 | 88,638 | 4,028,106 | 4,028,100 | **3,939,468** |
>
> **마스터 = 주석의 기적립액 · 5-3 표 = 주석의 잔액**으로 7/7 정확히 갈린다. 기적립액은 결산
> 확정치라 한 해 동안 안 움직이므로 **이월처럼 보였을 뿐**이다(48셀 중 46셀이 직전분기와 동일값).
> item5 의 개념은 잔액이므로(`ACCRUED_SUFFIXES` 에 `잔액` 포함, 항목8 주석 "실제론 합산액") 5-3 이
> 맞다. **owner 가 지적한 라이나 "2배" 는 그 분기 예정액이 우연히 기적립액과 비슷했던 것**이다
> (22,513 + 22,672 = 45,184).
>
> **채택 규칙 5개**(기중 · 필링간 1% 일치 · 자기분기 관측 존재 · 마스터와 1% 초과 차이 · 규모
> sanity) → 채택 49 · 보류 124. 이후 **주석 대조로 1건 철회 → 최종 48셀**.
>
> **철회 1건 — 농협생명 2024.1Q (owner 반문 덕에 발견).** 그 필링 주석(FY2024_Q1 p94)이
> `기적립액 1,954,358 + 예정금액 54,444 = 예정잔액 2,008,802` 이고 **마스터가 이미 그 예정잔액을
> 갖고 있었다**. 5-3 표의 1,981,300 은 기적립액도 예정잔액도 아니라 두 소스가 어긋나는 유일한
> 건이라 마스터를 유지했다. `bs_manual_overrides.json` `_removed` 에 사유 등재.
>
> **주석 대조 전수 결과**: `agree_balance 7 · disagree 1 · no_note 41`
> (`data/_derived/_probe_item5_notecheck.json`). **no_note 41 은 탐지 실패가 아니다** — 한화생명
> 3분기 공시는 38쪽 축약본이라 재무제표 주석 자체가 없고 5-3 표가 그 필링의 유일한 공시다.
>
> **추출기 자체 결함 2개를 스스로 적발**: ① docling MD 는 이 표를 통째로 떨어뜨리거나 라벨 열을
> 뒤로 민다(MD 172 vs RAW PDF 203표) → PDF 좌표 재구성(`fitz` words → y버킷 → x정렬).
> ② 단위를 페이지 첫 `단위` 줄에서 읽으면 표가 여러 개인 페이지에서 100배 틀린다(코리안리
> 2023.4Q) → 절 제목 사이로 창을 좁혔다. 필링간 충돌 12 → 9건.
>
> **눈으로 재확인한 원문 5건**: 한화생명 FY2025_Q4 p52 · 라이나 FY2024_Q1 p18+p67(주석) ·
> 한화생명 FY2026_Q1 p25(26.1Q **71,087** = 빌더 주석이 인용한 보도치 7조1,097억) ·
> KB라이프 FY2023_Q1 p15 · 농협생명 FY2024_Q1 p94(철회 근거).
>
> **반영**: `data/dart/viz/bs_manual_overrides.json` 18→66셀. 재빌드 후 전수 diff **변경 48셀 =
> 의도 48셀 · 항목5 외 변경 0 · 행수 6858 불변**.
>
> **업권 합계와 보도치**:
>
> | 앵커 | 보도치 | 수정 전 | 수정 후 |
> |---|---|---|---|
> | 2023년말 | 32.2조 | 31.92조 (-0.9%) | 31.92조 (불변) |
> | 2024.6말 | 38.5조 | 36.87조 (-4.2%) | **40.19조 (+4.4%)** |
> | 2026.6말 | 58.1조 | 60.96조 (+4.9%) | 60.96조 (불변) |
>
> ⚠ 2024.6말 편차가 **-4.2% → +4.4% 로 부호가 뒤집힌다**(±5% 종결조건 안이지만 개선은 아니다).
> 기중 공시치는 예정액을 포함한 진행 추정이고 Q4 는 결산 확정치다 — 2024.3Q 42.89조가 2024.4Q
> 37.99조로 떨어지는 패턴이 그 성격이다. 보도치의 기준 시점에 따라 달리 읽힐 수 있어
> **owner 판단 대기 항목**.
>
> **부수 정리**: `statutory_reserve_baseline.json` R-RSV-1 3건(한화생명 ×2 · DB생명)이 실제
> 해소돼 제거(`_resolved_log` 보존). 케이디비생명 1건은 이전부터 낡아 있던 것이라 미변경.
>
> **게이트**: `validate_data_contract` RED=0 exit=0 · `validate_master_tables --no-build` exit=2 는
> **수정 전과 SUMMARY 글자 그대로 동일**(백업본 직접 대조 — CSM/PL 축 기존 결함, 17BS 무관) ·
> xlsx `17BS` cherry-pick 동기화. **`prepush_check` 가 RED=2 로 한 번 막았다** —
> 골든 fixture 만 `--update` 하고 짝인 **입력 지문**(`validate_golden_input_fingerprints.py --update`)을
> 안 올렸기 때문. 정당한 차단이고 둘은 항상 같이 갱신해야 한다.
>
> **남은 것**: 보류 124건 중 비교열만 있는 30건, 마스터에 셀이 없는 47건(대부분 0=미적립)은
> 쓰지 않았다. 커버리지 확대는 별건.
