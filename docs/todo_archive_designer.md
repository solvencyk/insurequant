...
> `TODO_designer.md` 의 Status 이력 블록을 2026-09-11 에 **한 글자도 고치지 않고** 옮긴 것(최신순). 세션 시작 시 읽지 않는다 — `docs/changelog_*.md` 처럼 특정 과거 결정의 배경이 필요할 때만 연다. 활성 TODO 의 Status 에서 밀려난 항목은 이 줄 바로 아래에 그대로 잘라 붙인다(최신이 위).
...
- **원인**: `PL_STEP_DEFS` 에 하이픈 항목(`2-1`·`3-1`·`8-1`)이 없어 코리안리재보험(KR1000)의
  **장기보험 leg 이 통째로 안 그려지고** 브리지가 앵커(보험손익)와 안 닫혔다(2026.2Q 143.0억 = item2-1 전액).
- **분기 기준은 회사코드가 아니라 하이픈 항목의 존재.** `plLobSuffixes()` 가 그 (회사,기간)의
  `3-N`/`8-N` 중 값이 있는 접미사를 모아 `[]` 가 아니면 LOB head 로 갈아 끼운다. 근거: 하이픈 블록
  값은 **이미 item1 에 들어가 있어 안 그리면 산수가 틀린다** — 판단 근거가 "재보험사인가"(취향)가
  아니라 "이 기간 그 슬롯에 값이 있는가"(사실)가 된다. `-2`·`-3` 이 들어와도 축이 자동으로 늘어나고,
  하이픈 없는 회사는 스캔이 빈 배열이라 **기존 `PL_STEP_DEFS` 객체를 그대로 반환**(회귀가 구조적으로 0).
- **스텝** = `생명 수재(3) → 생명 출재(8) → 장기 수재(3-1) → 장기 출재(8-1) → 일반(14) →
  기타(15−16) → [보험손익]`. **자동차 막대 없음** — item13 이 14분기 전부 null 이라 `optional` 로
  빠진다(미해당 ≠ 결측, 회색 "미공시" 막대도 안 띄움 — 게이트 `lob_na` 축 `b2f9660`).
- **마스터 항목명 무수정.** 회사코드가 남는 곳은 화면 라벨 한 곳뿐(`PL_LOB_DISPLAY = {KR1000:{"":"생명","1":"장기"}}`).
  기본 블록 항목명이 `생명장기 …`(스키마 전역 슬롯 이름)라 실제 LOB(생명)을 데이터에서 못 읽기 때문.
  매핑 없는 회사용 **폴백**: 소계 항목명에서 꼬리 `손익` 만 뗀다(`장기재보험 손익` → `장기재보험 수재`).
  근거를 `docs/domains/claude-agent-ifrs17.md` §4.1b 인용과 함께 코드 주석에 남김.
- **세부 4개는 차트에서 접고 표에서 토글로.** 하이픈 블록도 세부가 4개씩(4-1~7-1·9-1~12-1) 있어
  일관되게 막대로 두면 16막대가 된다 → 차트 6막대, 표는 네 다리 각각에 ＋ 토글(`　└ CSM상각/위험조정/
  예실차/기타`). 토글을 `d.sub` 로 일반화했고 기본형(생명장기재보험 1개)은 DOM·문구가 종전과 동일.
- **실측(전 분기)**: 코리안리 14분기 x 2모드 스텝 합 vs 앵커 **최대 |잔차| 0.03억**(반올림). item1 자체가
  결측인 5칸(2023.1Q/2Q·2023.3Q 당분기)은 기존 상태라 잔차 정의 불가.
- **회귀(수정 전/후 두 페이지 대조)**: `plBuildSteps` 712조합 중 diff **28건 전부 KR1000**,
  `renderPlTable` 라벨·토글 diff **0**. 전사 renderCompany 순회 **JS 에러 0**,
  `pytest tests/test_deploy_assets.py` 10/10.
- **부수 수정 1건**: `samo()` 가 `-0` 을 뱉고 있었다(`Math.round(-0.4)===-0`, `-0<0` 이 false 라 △ 분기를
  안 탄다 — △ 표기 LOCKED 룰 위반). 코리안리 `장기 출재 → 위험조정 2026.2Q` 에서 노출돼 발견.
  `Math.round(n)||0` 로 정규화, 전수 영향 **43셀 `-0`→`0`** 뿐.
- **부수 발견(안 고침)**: Panel 5 y축 눈금 라벨 중복 — 눈금 0.05조인데 포매터가 `maximumFractionDigits:1`
  이라 `0.1` 이 두 번 찍힌다. **기존 이슈**로 전수 213건 중 **197건이 비KR1000**이라, 고치면 이 티켓의
  "다른 회사 회귀 0" 기준과 충돌해 손대지 않음. orchestrator 판단 요청(티켓 §6).
- **배포·push 안 함.**
