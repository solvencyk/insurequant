...
| **UH-25** (신규 2026-09-14) | **비-PDF 출처의 posted 행은 값 검증 두 축이 *둘 다* 안 돌고, 그 사실이 exit code·배포본·화면 어디에도 안 남는다.** `check_esr_in_source.py::check_row` 의 `is_pdf` 분기가 비-PDF 를 `scan_landing` 으로 보내 `verdict=skip_landing`(`ESR_VERDICT_YELLOW` = 인쇄만) + `adjusted_verdict=not_applicable`(`ESR_ADJUSTED_ABSTAIN` = 기권)으로 적는다 → `JP_ESR_NOT_IN_SOURCE` 도 `JP_ESR_ADJUSTED_FIGURE` 도 **침묵**한다. **사본 드라이런 실측(S8)**: 第一ライフグループ 220% 를 비-PDF URL 로 posted 로 뒤집으면 빌더 **exit 0 · RED 0건**(6종 중 5종 침묵, NOT_IN_SOURCE 만 YELLOW 한 줄). **화면상 구분 불가**: 그 산출 `jp/jesr_esr.json` 에서 0축 검증 행과 2축 검증 행의 **키 집합 차이가 공집합**이고, `jp/jesr_app.js` L307 은 둘 다 같은 「根拠資料 ↗」 링크로 그린다 — 즉 **"검증 못 함" 과 "검증 통과" 가 사용자에게 같은 모양**이다(이 저장소가 이름 붙인 false-green). **이미 1건이 라이브다**(0 아님): posted 15사 중 `skip_landing`·`not_applicable` = **T&D 222%** 1사이고, 그 증거행 evidence 는 「본문에 222 표기가 **안 보인다**」 인데도 게이트는 green. **비대칭이 근거**: 같은 YELLOW 인 조정치 축은 `test_live_esr_evidence_has_no_unexempted_adjusted_figure` 로 push 묶음에 이빨이 있는데, skip 축에는 라이브 대조가 **하나도 없다** — 유일한 테스트 `test_skip_verdicts_are_yellow_not_red` 는 *침묵을 단언한다*. 게이트 요약도 조정치 축만 분포를 찍고(`unqualified=7 … not_applicable=1`) 1차 축(found/not_found/skip_*)은 **분포를 안 찍는다** — 「기권은 세지 않으면 사각이 된다」 를 조정치 축에만 적용했다 | **등재만 했다. 룰은 안 만들었다**(UH-5·UH-9 선례 — 오탐 억제 설계 가능성 실측이 선행). **오탐 억제는 설계 가능해 보인다(단정 아님)**: 필요한 필드가 이미 박제돼 있다 — `esr_in_source_health.json` 의 `doc_kind`(실측 pdf 14 · other 1)·`verdict`, `source_url_health.json` 의 `content_type`(실측 application/pdf 52 · text/html 194 · 없음 8). **새 수집기 없이 셀 수 있다.** 막는 것은 **구조적 긴장**이다 — T&D 가 비-PDF 인 이유는 종전 TDnet URL 이 만료돼 `JP_SOURCE_EXPIRING_HOST` 를 피하려고 상설 IR 페이지로 옮긴 것이라, 지금 RED 로 걸면 **데이터 오류가 아닌 1건이 정상 배포를 막는다**(고칠 대상이 URL 인지 룰인지 미확정). **10/31 노출은 크다**: not_yet 62행이 지금 들고 있는 URL 은 ir_url 60/62 비-PDF · disclosure_url 비-PDF 33 · pdf 5 이고, 有報 스윕이 쓰는 EDINET 뷰어 URL(`disclosure2.edinet-fsa.go.jp/WZEK0040.html?…&S100YC7A`)도 **.html** 이다 — 第一ライフグループ 근거문서가 바로 그 docID 라 이번 라운드에 2건째가 생길 수 있다. 검토 방향 3안(싼 순서): ① 라이브 증거에 `skip_*` 상한을 거는 테스트(면제 레지스트리 경유) ② 1차 축 판정 분포를 게이트 요약에 인쇄(조정치 축과 대칭) ③ 배포본 record 에 검증상태 필드를 실어 **화면이 "미검증" 을 말하게**(불변식 1 — 검사한 파일과 보는 파일을 같게). 10/31 재census 에서 재측정 필수 / **P1** |
