...
| [PM-2026-09-13](PM-2026-09-13_jp_secondary_source_and_dead_url.md) | **jp 레인이 2차보도·조정치를 헤드라인으로 올렸고 출처 URL 은 무관한 문서였다** — 東京海上HD 238(원문 268) · かんぽ 220(「大量解約リスクを除いた場合」 조정치, 원문 181) · 明治安田生命 208.0(원문 208.7, 출처가 PDF 아닌 디렉토리) · MS&AD 출처가 합병 보도자료(ESR 미수록). jp 빌더 self-check(범위·형식·합계)는 **전부 통과** — census 안에서만 닫히는 자기참조라 '출처가 살아 있나 / 그 문서에 그 숫자가 있나' 축이 없다 | ❌ **미배선.** 도구는 신설(`check_source_urls.py`·`edinet_esr_probe.py`)했으나 어느 게이트에도 안 걸려 있다 | `open` (UH-18) |
...
| **UH-18** | **jp 레인 출처·수치 룰 4종(`JP_SOURCE_URL_DEAD`·`JP_SOURCE_EXPIRING_HOST`·`JP_ESR_NOT_IN_SOURCE`·`JP_ESR_EDINET_MISMATCH`)이 어느 게이트에도 안 걸려 있다.** 도구는 있다(`J-ESR/check_source_urls.py` exit 1, `J-ESR/edinet_esr_probe.py`) — 사람이 기억해서 돌려야 도는 honor system 이고, 이 저장소는 UH-14·UH-15 에서 같은 실패를 이미 기록했다 | 10/31 재census 는 62사가 한꺼번에 posted 로 뒤집히는 라운드다. 오늘 3사에서 3건이 나왔다. **배선 방향**: 오프라인 가능한 EXPIRING_HOST 는 빌더 self-check 에 즉시, 네트워크가 필요한 나머지는 **증거 신선도 검사**(`source_url_health.json` 의 `checked_at` 이 census `checked_at` 보다 오래되면 RED)로 우회. 티켓 `inbox/jp/20260913T1500Z__validation__JP_MULTI__jp_source_gate_wiring.md` / **P1** |
