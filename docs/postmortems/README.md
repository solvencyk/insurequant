...
| [PM-2026-09-13](PM-2026-09-13_jp_secondary_source_and_dead_url.md) | **jp 레인이 2차보도·조정치를 헤드라인으로 올렸고 출처 URL 은 무관한 문서였다** — 東京海上HD 238(원문 268) · かんぽ 220(「大量解約リスクを除いた場合」 조정치, 원문 181) · 明治安田生命 208.0(원문 208.7, 출처가 PDF 아닌 디렉토리) · MS&AD 출처가 합병 보도자료(ESR 미수록). jp 빌더 self-check(범위·형식·합계)는 **전부 통과** — census 안에서만 닫히는 자기참조라 '출처가 살아 있나 / 그 문서에 그 숫자가 있나' 축이 없다 | ⚠️ **부분 배선** (2026-09-13 UH-18). `build_jesr_page_json.py::source_gate_check` 에 4종 — `JP_SOURCE_EXPIRING_HOST`(netloc ∈ `jesr_http.EXPIRING_HOSTS`, import) · `JP_SOURCE_URL_DEAD`(증거 기록 판독) · `JP_SOURCE_EVIDENCE_STALE`(증거 부재·scope≠all·census 보다 낡음) · `JP_SOURCE_EVIDENCE_INCOMPLETE`(posted URL 미점검) → `self_check` 경유 **exit 1 실증**. 예외 등재처 `J-ESR/jp_source_exceptions.json` 신설(0건, owner 권한). 회귀 `tests/test_jp_source_gate.py` 43 + 이빨 5/5 | `open` (UH-18 부분 해소 · **UH-19 신규**) |
...
| **UH-18 ⚠️ 부분 해소 (2026-09-13)** | 4종 배선 완료 — `J-ESR/build_jesr_page_json.py::source_gate_check` → `self_check()` errors → `main()` **exit 1**. 오프라인/온라인을 갈라서 걸었다: netloc 만 보면 되는 `JP_SOURCE_EXPIRING_HOST` 는 빌더가 직접 보고, 네트워크가 필요한 판정은 선행 단계(`check_source_urls.py --all`)가 `source_url_health.json` 에 **박제**하고 빌더가 그 박제를 읽는다(`JP_SOURCE_URL_DEAD`). 박제가 낡거나(census 최신 `checked_at` 보다 이전) 좁거나(`scope != "all"`) 빠지면(posted URL 미수록) RED — UH-14 의 "정본 증거가 push 묶음 밖" 을 **증거의 나이**로 우회한 형태. 오탐 억제는 실측 기반: RED 로 읽는 분류는 `dead` 하나뿐(전수 254건에서 `blocked` 20·`ok_requires_headers` 16·`tls_client_issue` 4·`spa_shell` 4·`error` 4 = 48건이 거짓 RED 가 될 뻔했다). 예외 등재처 `J-ESR/jp_source_exceptions.json` 신설(0건, 등재는 **owner 권한**, 절차 룰 2종은 면제 불가). 회귀 `tests/test_jp_source_gate.py` **43 케이스** + 이빨 검증 5/5 발화 | **남은 것은 `JP_ESR_NOT_IN_SOURCE`** — "그 문서에 그 숫자가 있나" 축. 사고 3건 중 **2건(東京海上HD 238 · かんぽ 220)이 정확히 이 축**이었고 둘 다 URL 은 살아 있었다. 배선 전에 오탐 억제 실측(이미지형 PDF·랜딩 페이지 정본 분포)이 선행돼야 한다 — UH-5·UH-9 선례대로 억제를 설계 못 하면 배선하지 않는다. 산출 규격은 PM §4c-pre 에 확정(`esr_in_source_health.json`, `source_url_health.json` 과 같은 봉투 → 신선도 검사 재사용) / **P2** |
