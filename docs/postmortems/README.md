...
| [PM-2026-09-21](PM-20260921_kics_sens_iqp_referenceerror.md) | **배포 HTML 의 JS 가 통째로 무검사였다** — designer 커밋 `2dbc4ca` 가 `K-ICS.html` 의 `function IQP(){…}` 정의 **한 줄**만 지우고 호출부 2곳을 남겨 라이브 금리민감도 패널이 `ReferenceError` 로 사망. 실측 138버킷 중 129 · **기본화면 39/39 사**. 데이터는 100% 정상 = 고칠 셀 0개, 순수한 렌더링 사망. 게이트는 전 단계 통과였고 owner 가 눈으로 잡았다 — 불변식 1번을 데이터 축에서만 지키고 화면 축에서는 한 번도 안 지킨 자리 | ✅ **push 게이트(훅 §1f) `DEPLOYED_JS_UNDEFINED_CALL`** — `scripts/validate_deployed_js.py` → `prepush_check.py` L409(호출)·L581(`blocked`)·L600(exit 2), FULL 전용(루트 .html/.js 가 §0 에서 전체 게이트 강제). 현 트리 오탐 **0**(토큰 73,175 · 참조 3,627) · 정의삭제 변이 **172/183 검출** · killer 변이 6종 · 실행으로 exit 2 확인 | `closed` (잔여 **UH-26** 헤드리스 런타임 · **UH-27** 라이브축 비차단) |
...
| **UH-13 ✅ 해소 (2026-08-30, owner 지시)** | **`data/dart/viz/csm_waterfall_history.json` 은 아무도 재생성하지 않는 정적 스냅샷이다.** 선언 빌더 `scripts/ifrs17_batch_historical.py` 가 2026-06 에 아카이브된 뒤 마스터만 백필·정정을 받아 벌어졌다. 실측 2026-08-25: 대조 1,581셀 중 **933건(59.0%) drift**, 최대 Δ 43,852억(삼성화재 2023.3Q closing) · 스냅샷 자체 단계 항등식 파탄 41건 · 마스터에 있는데 스냅샷에 없는 회사 14사. IFRS17.html 워터폴 이력 패널이 그 값을 그린다 | **해소: 파일을 삭제했다.** 2026-08-30 owner 지시로 `data/dart/viz/csm_waterfall_history.json` 을 저장소에서 제거했다 — 재생성 빌더(`scripts/ifrs17_batch_historical.py`)가 이미 아카이브돼 되살릴 경로가 없었고, 게이트가 그 **죽은 사본을 검사해 YELLOW 976건**을 만들고 있었다. 「마스터 파생으로 교체」 권고 대신 **소비처를 마스터로 돌리고 사본을 없애는** 쪽으로 닫혔다 — drift 가 구조적으로 0 이 되는 것은 같고, 유지할 사본이 하나 줄었다(UH-15 의 '사본이 느는 것을 탐지하는 장치가 없다' 와 반대 방향). **2026-09-14 재확인**(오케스트레이터): 파일은 브랜치·`main` 어디에도 없고, `IFRS17.html` 의 워터폴 Panel 1·6 은 `ix.wfx` = 루트 마스터 `CSM_waterfall.json`(2026.2Q 까지)을 읽는다. 그 파일을 읽는 게이트도 없다(`validate_master_tables.py` 의 언급은 주석 한 줄). owner 라이브 QA 로도 2026.2Q 표시 확인. **주의 — 이름이 비슷한 별개 파일이 있다**: 소문자 `data/dart/viz/csm_waterfall.json` 은 FY2024 연차 기준의 **다른 파일**이고 워터폴 패널을 그리지 않는다(`PATHS.wf`, `IFRS17.html` 주석에 「내용이 다른 별개 파일」 명시). 2026-09-14 에 오케스트레이터가 이 둘을 혼동해 「라이브 워터폴이 6분기 뒤처졌다」 고 잘못 보고했다 — **이 행을 읽는 다음 사람은 두 파일을 구분해야 한다.** 이전 기록(1,581셀 중 933건 drift 등)은 삭제된 스냅샷에 대한 것이지 지금 라이브에 대한 것이 아니다. PM-2026-08-25 §5 |
...
| **UH-25 ✅ 해소 (2026-09-14) — 게이트 축 + 화면 축 둘 다** | **비-PDF `source_url` 인 posted 행은 값 검증 두 축이 *둘 다* 안 돌고, 그 사실이 exit code·배포본·화면 어디에도 안 남았다.** `check_esr_in_source.py::check_row` 의 `is_pdf` 분기가 비-PDF 를 `scan_landing` 으로 보내 `verdict=skip_landing`(`ESR_VERDICT_YELLOW` = 인쇄만) + `adjusted_verdict=not_applicable`(`ESR_ADJUSTED_ABSTAIN` = 기권)으로 적는다 → `JP_ESR_NOT_IN_SOURCE` 도 `JP_ESR_ADJUSTED_FIGURE` 도 **침묵**하고 빌더는 exit 0 · RED 0. **비대칭이 근거였다**: 같은 YELLOW 인 조정치 축은 `test_live_esr_evidence_has_no_unexempted_adjusted_figure` 로 push 묶음에 이빨이 있는데, skip 축에는 라이브 대조가 **하나도 없었다** — 유일한 테스트 `test_skip_verdicts_are_yellow_not_red` 는 *침묵을 단언한다*. 2026-09-13 에 **T&D 222% 1건이 실제로 그 상태로 라이브**였다(증거 evidence 는 「본문에 222 표기가 안 보인다」 인데도 게이트 green). 게이트 요약도 조정치 축만 분포를 찍고 1차 축은 안 찍었다 | **✅ 게이트 축 해소 — `JP_ESR_UNVERIFIED_VALUE` 정의·배선 완료(2026-09-14).** 판정식 정본 = `build_jesr_page_json.py::unverified_value_reasons()` (게이트도 테스트도 **이 함수 하나**를 부른다 — 재타이핑 금지). **논리합**이다: (a) `verdict ∈ ESR_VERDICT_YELLOW`(skip_landing·skip_no_text) **또는** (b) `adjusted_verdict ∈ ESR_ADJUSTED_NOT_JUDGED`(not_applicable). 곱(=0축)으로 걸면 수집기가 반쪽만 회귀한 상태가 빠져나간다. 호출은 `_esr_in_source_check` 의 **verdict 분기보다 앞**이라 회사·verdict 필터 없이 전 행을 돈다. **오탐 억제의 선은 `abstain_no_prose` 를 (b) 에서 뺀 것이고 실측이 근거다** — 그건 PDF 를 읽고 「라벨동반 산문 조각 0개」 로 판정한 결과(표·차트 전용)라 1차 축이 `found` 로 살아 있다; (b) 에 넣으면 **정상 8사가 거짓 발화**(2026-09-14 실측, posted 16사 중). severity 는 **YELLOW**(빌더 exit 0)고 이빨은 조정치 축과 **같은 자리** — 배포본 증거에 면제 없는 발화가 남으면 `tests/test_jp_source_gate.py::test_live_esr_evidence_has_no_unexempted_unverified_value` 가 push 묶음에서 막는다. 면제는 `J-ESR/jp_source_exceptions.json` 에 `JP_ESR_UNVERIFIED_VALUE` 로 등재(**owner 권한**, 셀 단위, fail-closed, `expires_on`); 현재 **0건**. 기존 `test_skip_verdicts_are_yellow_not_red` 와 **역할을 docstring 에 갈라 적었다** — 그쪽은 *빌더 exit code 가 skip 으로 안 바뀐다*(합성), 새 것은 *배포본 증거에 그런 행이 안 남는다*(실데이터). 모순 아님. remedy ② 도 같이 닫았다: 요약이 `1차 축 판정 분포` + `값검증 축 census(판정됨/무검증/면제)` 를 조정치 축과 **대칭으로** 인쇄한다 | **배선 시점 실측 발화 0**(전제): `verdict {found:16}` · `adjusted {abstain_no_prose:8, unqualified:8}` · `doc_kind {pdf:16}` · 0축 **0사** — T&D 출처가 목록 페이지 → 統合報告書 PDF 로 바뀌어 점유가 사라진 창에 걸었다(UH-5·UH-9 선례). 배선 후 실데이터 `YELLOW 0건 · RED 0건 · 판정됨=16 무검증=0`. **회귀 85 → 101** · **변이 10/10 발화**(사본에서만, 원본 md5 `fe0021bc…` 복원 확인): 두 필드 **각각**(M1 verdict·M2 adjusted_verdict)·둘 다·skip_no_text·다른 회사·면제 등재로 침묵·다른 회사 면제는 여전히 발화·다른 룰 면제도 발화·진짜 빌더 엔드투엔드 exit 0. **중복 아님(M6)**: M3 변이를 남긴 채 새 라이브 테스트만 지우면 **나머지 100건 전부 통과**. **훅 실증**: 진짜 증거를 일시 변이 → `prepush_check.py` 가 `1 failed, 257 passed` · **`BLOCKED`**, 복원 md5 동일. **⚠ 잔여 = 화면 축(미배선)**: 배포본 `jp/jesr_esr.json` 에서 0축 검증 행과 2축 검증 행의 **키 집합 차이가 여전히 공집합**이고 `jp/jesr_app.js` L307 이 둘 다 같은 「根拠資料 ↗」 로 그린다 — 게이트는 막지만 **사용자는 여전히 "검증 못 함" 과 "검증 통과" 를 구분 못 한다**(불변식 1 의 화면 판). designer·publishing 소관, 티켓 `inbox/jp/20260914T0740Z__validation__JP_MULTI__uh25_screen_axis_residual.md` / **P2** | · **화면 축도 같은 날 해소**: 배포본 record 에 `value_verified{state, reason}` 를 실어 `verified`/`unverified`/`exempt` 를 구분한다(`build_jesr_page_json.py::compute_value_verified()`). **판정을 재타이핑하지 않았다** — 게이트와 같은 `unverified_value_reasons()`·`load_source_exceptions()` 를 부른다. 화면(`jesr_app.js`·`index.html`)은 `unverified`/`exempt` 에만 `.verify-badge`(`?` 値 未検証 / `免` 確認対象外)를 띄우고 `reason` 을 title 에 원문 그대로 보여주며, 「根拠資料 ↗」 앵커의 `aria-label` 에도 붙는다 — **T&D 때 그 링크가 목록 페이지였는데도 '여기서 확인했다' 고 조용히 주장하던 것이 이 UH 의 출발점**이었다. `caveat-badge`(값은 검증됐으나 비교 불가)와 실루엣·글리프를 일부러 다르게 했다(주장이 다르다). 오늘 실데이터는 16/16 `verified` 라 화면 변화가 **안 보이는 것이 정상**이고, 그래서 fixture 로 4상태를 강제 렌더해 확인했다(오케스트레이터 독립: index 2면 + 상세 6케이스, 배지 수·title·aria·모바일 압착·가로스크롤 전건 통과). 증거를 흔들면 `unverified` 로 실제 뒤집히는 것도 실증. 배포 완료(main `25baed0`, 라이브 바이트 대조 5/5).
| **UH-24 ✅ 해소 (2026-09-16, owner Windows PC)** | `.githooks/pre-push` 가 **mode 100644**(실행권한 없음)로 커밋돼 있어 `core.hooksPath` 설정을 해도 git 이 훅을 조용히 건너뛴다(리눅스·macOS·Termux·클라우드 컨테이너 전부 해당). 게다가 훅이 owner Windows venv 경로를 하드코딩해서 **chmod 만 하면** 그 클론들에서 게이트를 한 줄도 안 돌리고 `exit 1`(push 전면 차단)로 오히려 더 나빠진다. 클라우드 세션은 `data/disclosure` 0개라 전체 게이트 자체를 못 돌려 검증 없이 "작업 PC 몫"으로 넘겼다(`docs/claude-changelog.md` 2026-09-14) | ① `git update-index --chmod=+x .githooks/pre-push`. ② 훅의 `PY` 를 owner Windows venv 우선·부재 시 `python3`→`python` 폴백으로 재작성(하드코딩 경로는 그대로 1순위 유지, 새 클론에서만 폴백이 발동) | 폴백 로직 시뮬레이션(가짜 경로 → `python3` resolve 확인) 후 **owner PC에서 훅을 실제로 두 번 끝까지 실행**(각 ~17-19분): 1차 `FAILED tests/test_jp_source_gate.py::test_gate_prints_the_adjusted_finding_end_to_end`(파생 버그, 아래) → 수정 후 2차 `567 passed, 2 skipped` · `PRE-PUSH VERDICT ... gate-clear` · `EXIT_CODE=0`. 훅이 건너뛰어지지 않고 **실제로 도는 것**까지 실측한 것은 이번이 최초다. **파생으로 잡은 실제 버그(같은 커밋에 동봉, 별도 UH 불요)**: 콘솔이 cp949 인 환경(owner Windows PC, 한국어 로케일)에서 `J-ESR/build_jesr_page_json.py::self_check` 가 일본어 산문(예: かんぽ生命保険)을 `print()` 하면 `UnicodeEncodeError` 로 죽는다 — 클라우드 컨테이너는 콘솔이 기본 UTF-8 이라 46개 커밋 내내 한 번도 안 걸렸다(이 자체가 UH-24 를 "작업 PC 몫"으로 미룬 판단이 옳았다는 증거). 수정: `build_jesr_page_json.py` 상단에 `sys.stdout/stderr.reconfigure(encoding="utf-8")`(`scripts/prepush_check.py` 와 같은 패턴) + `tests/test_jp_source_gate.py::_run_builder` 서브프로세스 호출에 `encoding="utf-8", errors="replace"`+`PYTHONIOENCODING=utf-8`(자식이 UTF-8 로 print 해도 부모가 로케일 기본값으로 디코드하려다 죽는 반대쪽 절반). 회귀: `tests/test_jp_source_gate.py` 101 passed |

| **UH-26** (신규 2026-09-21) | **런타임 전용 실패는 여전히 무검사다.** 신설된 정적 축은 "이름이 어디에도 없다" 만 본다 — `TypeError: a.b is not a function` · `getElementById()` 가 null 인데 `.style` 접근(엘리먼트 id 개명) · 차트 옵션 스키마 오류 · fetch 응답 모양 변화 · **스코프 오류**(IIFE 안에서 정의하고 밖에서 호출; 전수 변이 183건 중 9건이 이 이유로 미검출)는 안 잡힌다. 전부 "데이터는 맞는데 화면이 죽는" 같은 부류다 | **헤드리스를 차단축으로 쓰려면 선행조건 둘이 필요하다**: ① 배포본이 CDN 4종을 로드하는데 훅은 오프라인에서도 돌아야 한다(스텁을 쓰면 '진짜 페이지가 아닌 것'을 검사하는 것) ② **브라우저 없는 클론에서 SKIP 이 fail-open 이 되지 않는 설계** — 그것 없이 배선하면 '안 돌렸다'가 '통과했다'로 읽힌다(이 저장소의 반복 사고형태). 이 머신에서 헤드리스 반복 실행이 행으로 죽은 전례도 있다 | `inbox/validation/20260921T0630Z__validation__ALL__headless_runtime_smoke_feasibility.md` / **P2** |
| **UH-27** (신규 2026-09-21) | **라이브(`origin/main`) 축이 인쇄만 하고 push 를 막지 않는다.** 이번 사고의 본질은 *이미 배포된 것*이 하루 넘게 깨져 있었고 아무 기계도 그 말을 안 했다는 것인데, 새 게이트는 "내가 밀려는 것" 을 본다. 차단으로 걸면 designer 가 main 에 고칠 때까지 무관한 작업까지 전부 막히므로 일부러 정보로 뒀다 | `validate_deployed_js._live_audit()` 이 매 실행 `[라이브 감사 · 비차단] origin/main@<sha> RED=n` 을 **인쇄한다**(조용한 미배선이 아니다). 승격 대안 = 배포 직후 1회 `--git-ref origin/main` 을 launch-runbook 에 박기. owner 판단 | `inbox/publishing/20260921T0630Z__validation__ALL__live_main_still_broken_deploy_iqp_fix.md` / **P1**(지금 라이브가 깨져 있다) |
