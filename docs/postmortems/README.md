...
| **UH-21** (신규 2026-09-13) | **문서 안에 실재하지만 정의가 다른 값**을 가르는 축이 없다. `JP_ESR_NOT_IN_SOURCE` 는 "그 숫자가 그 문서에 있나" 만 묻기 때문에 かんぽ 220%(「大量解約リスクを除いた場合」 조정치)를 `found` 로 통과시킨다 — 실측: p35, d=1. 즉 **PM-2026-09-13 의 사고 3건 중 1건이 아직 어떤 룰에도 안 걸린다**. 재발이 쉬운 형태다 — 일본 공시 자료는 헤드라인 옆에 「○○を除いた場合」·「適正水準 150~220%」·「調整後」를 나란히 싣는 것이 관행이라, 표에서 한 칸 옆을 집으면 형식·범위·출처가 전부 맞는 채로 틀린 값이 실린다(census notes 에 조정치임을 **스스로 적어 놓고** 그 값을 헤드라인으로 쓴 것이 실제 사고다) | **오탐억제 실측 선행 완료**(UH-5·UH-9 선례대로): 판정식 후보 = 값이 ESR 라벨과 같은 문장 조각 안에 나오는 등장 중 한정어(`を除いた場合`·`調整後`·`適正水準`·`ターゲットレンジ` 등) **없는 등장이 0개**면 조정치 후보 → YELLOW. 표본 검증: かんぽ 220 발화(한정어없는조각 0) / かんぽ 181 미발화(2) 로 **둘을 가른다**. **단 정상 14사 중 7사는 라벨동반 산문 조각이 0개**(표 행·차트 라벨 전용 문서)라 그대로 걸면 거짓 YELLOW 7건 — **"라벨동반 조각 ≥ 1 일 때만 판정, 0 이면 기권"** 조건이 필수다. 그 조건을 붙이면 표본 발화 **1건(かんぽ 220)**·오탐 0. severity 는 처음엔 YELLOW(한정어 목록이 휴리스틱). PM-2026-09-13 §5 / **P1** |
