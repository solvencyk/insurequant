...
오케스트레이터 2차 지시: (1) "기적립액+적립예정액=잔액"으로 개념을 통일하라(악사손해 표 형식이
분기마다 다르다 -- 표 2행/표 1행/표 1행+각주문장 세 형식이 섞여 있었다) (2) 허용오차를 상대
1%에서 절대 ±50백만으로 바꿔라(경영공시는 억원 반올림이라 정당한 차이는 그뿐).

`_narrative_totals()` 신설 -- 각주 문장에서 "적립할 경우...XXX억원"(최종잔액 직접명시) 또는
"적립예정액은...XXX억원"(증분만, 표기적립액과 합산) 두 문형을 파싱. 항목번호를 5(해약환급금)
하나에서 5/6/7/8(비상위험·대손·보증 포함) 딕셔너리로 일반화하며 회귀 1건 발견·수정(클러스터
경로가 한 셀 안의 여러 항목 중 첫 번째에서 멈추던 버그). "예정잔액"(어순 반대, AIA 대손준비금
전용) 접미사 추가.

4Q/연차 필링에서 **별도(standalone)/연결(consolidated) 중복 노트**를 발견 -- 같은 대손준비금
3행노트가 두 페이지에 따로 있고 값이 다르다(교보생명·삼성생명 실측). "연결법인"/"연결실체"
자기신원 문구로 연결 쪽을 감점, 별도를 결정론적으로 채택하도록 수정 -- 기존 충돌 9건이 전부
해소됨.

39사 x 전분기 x 4항목 전수 재대조(2,188칸): 절대차 50백만 초과 135건(item5=9·item6=26·
item7=66·item8=34), 비율 5~14배로 튀는 11건은 원문 직접 재대조로 개별 확인(전부 프로브가
맞고 마스터가 틀렸음을 확인). 형식 census: 표 단독행(각주없음) 88% 압도적 다수, 문장 기반은
전체의 0.3%(6건)뿐이지만 걸리면 개념이 통째로 절단되는 사각지대였다.

`data/dart/viz/bs_manual_overrides.json`에 132칸 신규 등재(cells 231→363, 삼성생명·한화생명
전 항목과 DB생명 item5 2026.2Q는 제외 -- 각각 오케스트레이터 소관/기존 owner override가 더
정확) → `build_ifrs17_bs.py` 재실행, 7,042행 무변 확인, 135/135 셀 재검증 통과. 골든·xlsx·
public_exports는 오케스트레이터가 별도 처리 중이라 이 커밋에서 제외.

상세: `TODO_parser_ifrs17.md` 2026-09-03(85th pass) 항목.
...
**탐색 2회차**: 생보 3사가 게시로 뒤집혔다(라이나 KR0074 · BNP카디프 KR0075 · 메트라이프 KR0095).
생보협회 일괄은 아직 2분기 미게시라 자사 사이트에서 받아야 했고, 그 경로가 없어
`scripts/download_disclosure_2026q2_life_sites.py` 를 신설했다(세 사이트가 서로 다른 스택이라
회사별 XPath — 라이나 `<button>`, BNP `a.filedown` 자체가 행 제목, 메트라이프 `.pdf` 앵커).
`verify_q2_disclosure_content.py` **4/4 ACCEPT**. 손보는 여전히 하나손해 1사뿐, 미관측 0.

**해약환급금준비금 비교 결과**:

- **2026.2Q 2건 교체**: 메트라이프 2,021,231 → **2,733,000**(+35.2%) · 라이나 3,939,468 →
  **4,027,700**(+2.2%). 둘 다 마스터가 2025.4Q 값을 소수점까지 복사하고 있어 2026.2Q 실측이
  없었다. 라이나는 owner 의 materiality 기준 아래지만 같은 복사 결함이라 함께 넣었다.
- **교차검증**: 두 필링의 `직전 분기` 열(23,130 / 40,281)이 앞서 고친 마스터 2026.1Q 와 정확히
  일치 — 48셀 작업을 다른 필링이 독립 확인했다.
- BNP 2026.2Q 는 `-`(미적립)로 마스터 0 과 일치, 하나손해는 0 이고 원문에 "별도의 준비금 적립
  사항은 없으며" 명시(legit zero). 둘 다 수정 불필요.
- **남은 차이는 무시 가능**: 미교체 셀 중 1% 초과는 9건뿐이고 최대가 분기 업권합계의 0.53%.

**신규 24칸 등재(owner 승인)**: 마스터에 셀 자체가 없던 (회사,분기)를 경영공시로 채웠다.
대부분 2023.1~3Q — 그 구간 모집단이 8~14사뿐이라 통째로 빠져 있던 회사들이다.
비교열만 있는 20건과 필링간 불일치 3건은 **쓰지 않았다**. 6칸은 그 분기에 다른 17BS 항목이
없는 단독 행인데, 게이트 코어는 항목 1~4 이고 5·6·7 은 owner 가 optional 로 못박아 둔 축이라
census 를 건드리지 않는다(실측 확인).

마스터 6,858 → **6,885행**. 전수 diff: 변경·추가 77셀 전부 항목5 · 오버라이드 값과 불일치 0 ·
누락 0. 예별손해 item5 2025.4Q~2026.2Q 3칸이 `None→0` 으로 늘었는데 내가 넣은 2025.3Q=0 의
빌더 이월이다.

**업권 합계**: 2023.1Q 11.86→18.92조 · 2023.2Q 16.33→24.36조 · 2023.3Q 20.15→29.34조.
종전 2023.3Q(20.15) → 2023.4Q(31.92) 의 계단이 29.34 → 31.92 로 자연스러워졌다.
2026.6말이 +4.9% → +6.3% 로 ±5% 를 벗어났으나 **owner 가 유지 결정**(2026-08-31): "raw 데이터에
확실히 있는 숫자면 굳이 기사 숫자에 안 맞춰도 된다. 걍 참고용이었다." → **보도치 앵커는 참고용
sanity check 이지 종결조건이 아니다.** 원문 값과 앵커가 어긋나면 원문을 남긴다. 2024.6말의
-4.2% → +4.4% 부호 전환도 같은 이유로 종결.

게이트: `validate_data_contract` RED=0 exit=0(법정준비금 신규 RED=0) · 골든 + 입력지문 동시 갱신 ·
xlsx `17BS` cherry-pick 동기화(요약 행수 6858→6885 자동 보정).

## 2026-08-31 — item5 기중값 48셀 교체 (경영공시 원문, owner 지시)

17BS item5(해약환급금준비금)의 **기중 분기값이 주석의 `기적립액`만 담고 있었다**(개념은 `잔액` =
기적립액 + 적립예정금액). K-ICS 정기경영공시 PDF 의 `5-3./7-2. 해약환급금준비금 등의 적립`
표(=주석의 잔액)에서 RAW PDF 좌표 재구성으로 읽어 **48셀**을 교체했다. 상세·근거·업권합계 대조는
`TODO_parser_ifrs17.md` 2026-08-31 항목.

- **원인 정정**: 처음엔 "연말값 이월"로 진단했으나 owner 의 반문("라이나는 딱 2배인데 의심스럽지
  않니")을 받아 주석과 대조해 보니 **개념 절단**이었다. 라이나 7/7 에서 마스터 = 주석 기적립액,
  5-3 = 주석 잔액으로 정확히 갈린다. 기적립액은 결산 확정치라 한 해 동안 안 움직여 이월처럼 보였다.
  라이나 "2배"는 그 분기 예정액이 우연히 기적립액과 비슷했던 것(22,513 + 22,672 = 45,184).
- **철회 1건**: 농협생명 2024.1Q — 그 필링 주석이 예정잔액 2,008,802 을 명시하고 마스터가 이미 그
  값을 갖고 있었다. 5-3 의 1,981,300 은 어느 쪽과도 안 맞아 마스터 유지(`_removed` 등재).
- **쓰기 경로** `data/dart/viz/bs_manual_overrides.json` (18→66셀). 재빌드 후 전수 diff 로
  **변경 48 = 의도 48 · 항목5 외 변경 0 · 행수 6858 불변** 확인.
- **주석 대조 전수**: agree_balance 7 · disagree 1 · no_note 41(축약 분기공시라 주석 자체가 없음).
- **추출기 자체 결함 2개를 스스로 잡았다**: ① docling MD 는 이 표를 통째로 떨어뜨리거나 라벨 열을
  뒤로 민다(MD 172 vs PDF 203표) → RAW PDF 사용. ② 단위를 페이지 첫 `단위` 줄에서 읽으면 표가
  여러 개인 페이지에서 100배 틀린다(코리안리 2023.4Q) → 절 제목 사이로 창을 좁혔다.
- **채택 규칙 5개**(기중 · 필링간 1% 일치 · 자기분기 관측 존재 · 마스터와 1% 초과 차이 · 규모 sanity)
  → 채택 49 / 보류 124. 보류는 쓰지 않았다.
- `statutory_reserve_baseline.json` R-RSV-1 3건(한화생명 ×2 · DB생명)이 실제 해소돼 제거.
- 게이트: `validate_data_contract` RED=0 exit=0 · `validate_master_tables --no-build` exit=2 는
  **수정 전과 SUMMARY 동일**(백업 대조 확인, 17BS 무관) · xlsx `17BS` cherry-pick 동기화.
- **`prepush_check` 가 RED=2 로 한 번 막았고 그게 옳았다**: 골든 fixture 만 `--update` 하고 짝인
  **입력 지문**(`scripts/validate_golden_input_fingerprints.py --update`)을 안 올렸다.
  `INPUTS_MOVED` + `FIXTURE_MOVED` 두 RED. **골든과 입력 지문은 항상 같이 갱신한다.**
- **owner 판단 대기 1건**: 2024.6말 업권합계가 36.87조(-4.2%) → 40.19조(+4.4%)로 보도치 38.5조
  대비 부호가 뒤집힌다(±5% 안이지만 개선은 아님).

## 2026-08-31 — 코리안리 금융재보험 분리 가능성 조사 (owner 질문, 코드 변경 없음)

owner: "코리안리 실적을 생명/장기/일반으로 나눴는데 금융재보험(공동재보험·대량해지) 실적은
어디에 할당되나, DART 에서 발라낼 수 있나". 실측 후 **PL·CSM 분리는 불가로 종결**하고 근거를
`docs/domains/claude-agent-ifrs17.md` **§5.4** 에 기록. 추출기는 만들지 않았다.

- **분리됨**: FY2025 사업보고서(20260319001095)에 `전통형재보험/공동재보험` XBRL 축 존재,
  `CoReinsuranceMember` 값 셀 **996개**. 단 measure 는 예실차 계열 6종(예상보험금·위험보험료·
  보험금 예실차비율 / 예상유지비·예정유지비·유지비 예실차비율)뿐이고, 하위 축은 계약의 유형
  (생명보험·장기손해보험) × 포트폴리오 × 만기구간.
- **분리 안 됨**: 보험손익·CSM·보험수익에 이 축이 안 붙는다. 대량해지는 키워드 0회
  (영문 `Mass Lapse` 가 임원 내부교육 이력표 한 줄뿐)라 아예 불가.
- **일반(item14)에는 안 섞인다** — `CoReinsuranceMember` 가 생명보험·장기손해보험 축에만 있다.
  즉 섞이는 곳은 item2(생명) · item2-1(장기).
- **연 1회, 사업보고서 전용**: FY2023 0셀 · FY2024 0셀 · FY2025 996셀 · 2026.1Q 0셀 · 2026.2Q 0셀.
  FY2025 에서 신설된 공시라 시계열이 1개 시점뿐이다.
- **탐지기 함정 2건 기록**: ① DART 숫자 셀은 `<TD>` 가 아니라 **`<TE>`** — TD 만 매칭한 1차
  추출기가 0건을 냈고 TE 로 바꾸자 1,344셀이 나왔다. ② 한글 키워드 카운트로 축의 존재를
  판정할 수 없다(축 이름이 영문 XBRL 토큰).
- **오독 1건 정정**: 오케스트레이터가 마스터 항목명 `생명장기 손익` 을 내용으로 읽어 "생명과
  장기가 분리 안 돼 있다" 고 보고했다. 실제로는 **코리안리만 생명(2~12)/장기(2-1~12-1)/
  일반(14) 3분할**이고 항목명은 전 회사 공통 슬롯 이름이며 화면이 `PL_LOB_DISPLAY` 로 라벨을
  덮는다. §5.4 서두에 이 함정을 명시했다.

## 2026-08-30 (77th pass) — 민감도 2건 정정 + 가정민감도 마스터 신설

### 1) 라이나생명 — 유배당 소블록을 집고 있었다

한 필링에 캡션·헤더·행수가 **완전히 같은** 민감도 표가 8벌(① 유배당 / ② 무배당 × 별도·연결).
`_pick_sensitivity_block` 의 정렬 키가 전부 동점이면 마지막 `-line_no` 로 갈려 **문서에 먼저
나오는 표**가 뽑히는데, 라이나는 유배당이 먼저다. 그래서 기준 CSM 16.9억짜리 소블록의
민감도(자기 CSM 의 0.004%)가 화면에 실려 있었다. 진짜 표는 무배당 별도(기준 23,743.7억 =
마스터 기말 23,760.6억과 0.07% 일치).

수정: 동점 시 문서 순서 대신 **마스터 기말 CSM 앵커**. 단 **0.5 decade(약 3배) 안이면 동점
처리**한다 — 자릿수 제한 없이 넣었더니 교보생명이 별도(-6.0%)에서 연결(-5.6%)로 뒤집혔다.
이 저장소가 반복해 데인 연결/별도 혼용을 새 룰이 만들면 안 된다. 조인 뒤 교보 원복.

### 2) 에이아이지 — 가드가 **이 회사에서만 조용히 꺼져 있었다**

`_csm_totals_eokwon()` 이 마스터의 `원수사명` 을 그대로 키로 쓴다. 민감도 쪽은
"에이아이지손해보험", 마스터는 "AIG손해보험" 이라 앵커가 None 이 되고, 그러면 **단위 xref 도
"3배 초과면 값 null" 가드도 둘 다 안 돈다**(둘 다 앵커 필요). 그 결과 자기 CSM(928.1억)의
**343배**인 318,643억이 화면에 나가고 있었다. 별칭 7건을 붙여 앵커 복구 -> 318.64억.
삼성생명·KB라이프·코리안리는 단위판정이 default -> xref 로 승격됐고 **값은 전부 불변**
(기존 추정이 옳았다는 확인).

### 3) 가정민감도 마스터 신설 (owner 상시 규칙)

owner: "화면 있는 그래프들 다 마스터테이블에 담으라 했다". 전수 대조 결과 `IFRS17.html`
"7) 민감도(ΔCSM)" 패널만 대응 시트가 없었다. 마스터에 이미 **`금리민감도`** 시트가 있어
같은 것으로 오인돼 왔는데 **다른 표다** — 저쪽은 K-ICS 지급여력비율의 금리 ±bp 민감도,
이쪽은 IFRS17 보험가정 충격의 ΔCSM.

`scripts/build_csm_sensitivity.py` -> `CSM_sensitivity.json` **195행**(194 시나리오 +
미수록 1사 stub). 소스는 **화면이 읽는 그 파일**이다(불변식 1). 에이아이에이생명이 같은
충격을 두 벌 싣는 문제(재보험 경감 전/후로 추정, 원문 라벨 미확인)는 임의로 하나를 고르지
않고 `순번`으로 식별 + `비고`에 사유를 남겼다.

**게이트가 새 시트에서 바로 일했다**: 처음엔 조인 키가 유일하지 않다고
`PUBLIC_EXPORT_KEY_AMBIGUOUS` RED 로 막았다(값 비교를 조용히 건너뛰지 않았다).
`sync_master_xlsx_sheet.py` 도 컬럼이 바뀌자 REFUSE 했다 — 둘 다 설계대로.

### 4) 남긴 것

`inbox/parser/20260830T2000Z` — 에이아이에이 `IFRS17_BS.json` 항목5 가 2024.1Q~3Q 만 비어
있다(이웃 분기엔 값 있음, 같은 회사 항목7 은 그 분기에 값 있음). 빌더 재실행으로 **값 diff 0**
이라 마스터가 낡은 게 아니라 로직이다. 이 한 건이 owner 수용기준 2024.6월말 −6.2% 미달의
대부분이다(채우면 −4.2% 로 진입).
