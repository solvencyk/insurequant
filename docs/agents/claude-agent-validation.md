...
### 3.1 가설 형성 방법론 (owner 2026-08-24 지적 — 가설 5연속 오답 후)
...
으로만 적어라 — 결론으로 적지 말 것. 서브에이전트에게 원인을 대신 정해 지시하지 말고, raw를
쥔 쪽이 직접 판정하게 한다.

2026-08-21~24 TFI/tier2 작업에서 이 규율 없이 가설을 다섯 번 세워 다섯 번 다 틀렸다(전체
경위·각 가설의 반증 실측은 `docs/changelog_validation.md` 2026-08-24 항목). 룰 코드(`kics_json_rules.py`
등) 수정으로 이어지는 가설은 이 단계를 통과해도 **편집 전에 현행식·제안식을 같은 데이터에
나란히 걸어 닫힘/깨짐 양방향을 세는 전수 시뮬레이션**이 별도로 필요하다 — 1건 고치려다
129건 깨뜨릴 뻔한 실측이 같은 라운드에 있었다(같은 changelog 항목).
