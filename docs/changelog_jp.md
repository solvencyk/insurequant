...
## 2026-09-15 (35) -- F조 출처 교체, 貸借対照表 29/31사

- **あいおいニッセイ同和**: 자사 ディスクロージャー資料2026 법정 계산서류(BS p30/PL p33). MS&AD
  第16期 有価証券報告書 p205/p207 과 BS 16행·PL 18행 전건 일치. 항등식 diff 0.
- **보험료 자릿수 복원**: census 1,471,100/1,430,300 은 IR덱 億円 표기 ×100 이고 FY2024 는 前期比
  열 역산이라 이중 파생이었다. 법정치 1,471,193/1,430,364 로 교체 — 억 절사하면 정확히 종전 값이
  되므로 값 변경이 아니라 자릿수 복원. 근거는 census `converted_from` 에.
- **MS&AD HD: 싣지 않기로 결정.** 提出会社 単体 BS 는 자산 81%가 関係会社株式, PL 営業収益은 전액
  関係会社受取配当金, 正味収入保険料·保険引受収益·支払備金·責任準備金 행이 한 줄도 없다 — 보험
  대차대조표가 아니다. 並行開示情報(요약연결)은 監査 미수검 명기 + scope 불일치. 버블에서 HD 를
  뺀 owner 결정과 같은 논리.
- **ソニー損保: 못 열었다.** 자사 도메인 Akamai 403(수단 전부 실패), EDINET 提出会社 아님(코드리스트
  11,388행 전수 확인), 모회사 SFG 자료에 ソニー損保 単体 BS 없음. 후보 URL 은 찾았으나 **본문 미확인
  이라 census 에 넣지 않았다**(T&D 급 미검증 출처 사고 방지). 사람이 열어 확인 후 병합할 큐.
- 부수 확인: SFG p34 에서 199,523/173,896 의 라벨이 元受正味保険料 임을 재확인 →
  `premium_caveat.code="gross_direct"`·`ratio_caveat.code="ei_basis"` 둘 다 정확.

## 2026-09-15 (34) -- 남은 10사 2차, 貸借対照表 21→28사

1차에서 막힌 10사를 3개조로 재발사. E조 6사·H조 1사 전건 성공, F조(출처 교체 3사) 진행 중.

- capital·三井ダイレクト·東京海上ダイレクト: BS 가 前期金額/前期構成比/当期金額/当期構成比/増減率
  **5열 구성비 표**라 내 `MAX_COLS=3` 이 통째로 거부하고 있었다.
- 楽天: 5년추이표(p10/p73)·経理説明용 PL(p72)에 속았고 법정 ②損益計算書는 p107.
- セコム: `(57,428)` 처럼 완전괄호가 **음수가 아니라 내역 표기**. p78 각문
  (収入保険料73,360−支払再保険料13,360=差引60,000)으로 양수 확인.
- ペット＆ファミリー: 법정 계산서류가 스캔 이미지 → 페이지 PNG 직접 판독(출처에 명시), prev=11256.
- レスキュー: OCR 대신 같은 사이트의 **決算公告**(회사법 440조) PDF 가 실텍스트였다(당기/전기 2문서).
- **파서 버그 감사**: `val()` 이 완전괄호를 음수로 처리 → 이미 커밋된 21사를 전수 감사한 결과
  그 PDF 들엔 완전괄호 수치가 없었고 음수 19건은 전부 △/▲ 에서 온 진짜 값이었다. 오염 없음.
- 결과: 貸借対照表 28사 · 損益 5항목 이상 27사. 4축 전건 통과.

## 2026-09-15 (33) -- 손보 법정 BS/PL 추출, 貸借対照表 5→21사 · 損益計算書 5→23사

손해율 31사 중 26사가 `bs.status="not_obtained"` + PL 보험료 1개뿐이던 상태를 메웠다. 26사 PDF 를
전부 받아(126MB, `J-ESR/raw/fy2025_samples/others/`, gitignore) 법정 単体 계산서류를 추출.

- 병렬 4개조 중 3개조가 세션 한도(429)로 죽어, A조 7사만 에이전트 산출. 나머지 19사는 결정론적
  범용 파서로 직접 처리(`scratchpad/bspl/generic.py`).
- **PL 페이지를 값으로 앵커링**: `正味収入保険料` 행이 이미 화면에 나가는 (前期, 当期) 쌍과 정확히
  일치하는 페이지 = 법정 損益計算書. 표제 문자열로 고르면 5년추이표·連結·부속명세를 집는다(7사 중 6사 실측).
- **BS 는 勘定式 대응**: 資産=왼쪽/負債·純資産=오른쪽이 같은 줄이라 x 중앙 분할 없이는 4열로 섞인다.
- 검증 4축: 스키마 / `資産合計=負債合計+純資産合計` / 보험료 당기·전기 / **법정 PL 역산 손해율 대조**.
  4축이 全管協れいわ 사업비율 +1227.8pt 를 잡아냈고, 조사 결과 추출 오류가 아니라 원문이
  "単位未満を切り捨てて表示"인 초소형사라 재현 불가한 것이었다 → 4축을 규모 인지형으로 수정.
- 적재: `extracted_bs_values.json` 10→26사(tree 는 `extract_bs.build_tree` 재사용),
  `J-ESR/nonlife_pl_census.json` 신설 19사. 빌더 자기검사는 삭제가 아니라 화이트리스트로 확장.
- 미완 10사: BS 페이지 미검출 3 · PL 앵커 실패 2 · 보험료 전기 불일치 1 · 출처 교체 필요 3 · 스캔 1.
...
이 세션은 "회사망에서 막혔던 것을 클라우드에서 다시 본다" 를 전제로 굴러갔는데 ESR 축만 하고
**이 축을 빠뜨렸다**. owner 가 그걸 짚어서 재개했다. 실측 현황: census 손보·재보험 34사 중
`jp/jesr_detail.json` 에 손해율이 있던 회사는 5사뿐이고 29사가 비어 있었다.

**조사.** 29사를 A(대형·중견 10)·B(중소·특화 9)·C(다이렉트·펫·재보험 10) 3조로 나눠 jp-collector
병렬(Sonnet 5 ×3, 각 45분 내외). 판정을 `found` / `no_ratio_in_doc` / `not_applicable_holding` ·
`not_applicable_reinsurer` / `unreachable` 로 **갈라서** 받았다 — "못 찾았다" 와 "없다" 를 섞는 것이
이 저장소가 반복해 데인 실수라서.

| 판정 | 수 |
|---|---|
| `found` | 24 |
| `not_applicable_holding` | 2 (東京海上HD·SOMPO HD — 지주 자료에 그룹 단일 합산치 없음, 사업회사 세그먼트 수치뿐) |
| `unreachable` | 3 (共栄火災 SSL 인증서 체인 · 大同火災 도메인 WAF 403 · ヤマップ Nuxt3 SPA) |

FY2025 는 기확보 5사를 합쳐 **29사 전건**.

**막혔던 3사.**
- **日新火災 확보.** (17)이 「WebSearch 로 정확한 URL은 특정 — `nisshinfire.co.jp/ir/pdf/disclosure2026.pdf`
  — 그러나 curl 차단으로 원문 미열람, 손해율 실측 안 함」 이라 적은 바로 그 PDF 가 이번엔 200 이다.
  FY2025 61.1/36.5/97.6 (p81 「正味損害率、正味事業費率及びその合算率」 合計行 직접, 좌표 대조).
- **あいおいニッセイ同和 확보(우회).** 자사 사이트는 JS flipbook 뿐이라 PDF 가 존재하지 않는다.
  모회사 MS&AD HD 결산자료의 **単体 열**에서 64.5/32.6/97.1 확보.
- **共栄火災 여전히 막힘.** `kyoeikasai.co.jp` 전 도메인 SSL 인증서 체인 오류, python requests ·
  `curl -v` · WebFetch 세 방법 동일 실패. (17) 증상과 일치. 추정하지 않고 `unreachable` 로 남겼다.

**검산.** `合算率 == 正味損害率 + 正味事業費率`(±0.15, (17)~(22) 라인의 H02/P06 과 같은 축)을
전 회사·전 연도에 돌려 **69건 중 불일치 0**. 값이 맞다는 증명이 아니라 세 값이 서로 모순되지
않는다(옮겨적기 오류가 없다)는 증명이다.

**연도 키 불일치 — 조용히 10사를 잃을 뻔했다.** 3조가 키 형식을 제각각 썼다:
A·B `"FY2025"` / C `"2025"` / あいおい는 dict 가 아니라 float 하나 / MS&AD `"FY2025_jgaap"`·
`"FY2025_ifrs_ref"`. **발주 프롬프트에 형식을 안 박은 오케스트레이터 잘못이다.** 실측 피해:
`row["loss_ratio_pct"]["FY2025"]` 로 순진하게 읽으면 **19사만 잡히고 10사가 조용히 사라진다**
(정규화 후 29사). 병렬 fan-out 산출은 **스키마를 발주에 못 박거나 합치는 단계에서 강제**해야
한다는 사례로 남긴다. `J-ESR/merge_nonlife_ratio_census.py` → `J-ESR/nonlife_ratio_census.json`.

**같은 열에 놓으면 안 되는 값 4건.** 값을 고치지 않고 `caveat` 필드에 **왜 비교 불가인지**만 적었다.
- `トーア再保険` — 正味損害率 = 正味支払保険金 ÷ 正味収入保険料 로 **損害調査費(LAE)를 분자에 안 더한다**.
  다른 손보사는 전부 (正味支払保険金 + 損害調査費) ÷ 正味収入保険料 → 이 회사만 과소 표시.
- `ソニー損害保険` — **`E.I.損害率`**(경과보험료·발생손해액 기준)이지 正味損害率(수입보험료 기준)가
  아니다. 범위도 地震保険・自賠責 제외.
- `レスキュー損害保険` — 이미지 스캔 PDF. 자동 텍스트 추출이 세로쓰기 표 헤더에서 깨져
  (`正味損害率` 열 라벨이 `下KK人` 류로 파손) 페이지를 렌더링해 **육안 판독**한 값이다.
- `MS&ADインシュアランスグループHD` — 「２社合計（単純合計）」. 회사가 직접 공시한 표라 파생은
  아니지만 三井住友海上·あいおい를 따로 싣고 있으면 **중복 계상**. 같은 자료의 IFRS 참고치
  (65.5/28.1/93.6, 除く家計地震・自賠責)는 범위가 달라 `alt_values` 로 분리.

**오탐을 하나 걸러냈다.** `損害調査費` 문자열이 4사에서 잡혔지만 ジェイアイ傷害火災·第一アイペット·
日本地震再保険 **3사는 표준 산식을 명기한 것**이고 진짜 정의 차이는 トーア 1사뿐이다.
안 갈랐으면 정상 3사를 이상값으로 몰 뻔했다.

**미반영.** census·`jp/*.json`·화면 전부 손대지 않았다. 남은 결정 3건은 화면 구조라 owner 판단:
① caveat 4건을 별도 열로 뺄지 라벨만 붙일지 ② `jesr_detail.json` 이 10사 구조인데 24사를 어떻게 실을지
③ SOMPOダイレクト `source_url` 이 비율표 없는 분책(21p)을 가리킨다 — **다만 그 행은 `not_yet` 이라
게이트 검사 대상이 아니어서 T&D 같은 false-green 이 아니다**(오케스트레이터가 처음에 같은 급으로
말한 것을 정정). census `checked_at` 을 한 칸이라도 건드리면 `_census_checked_at_max` 가 전 행을
훑으므로 수집기 2종이 다시 돌아야 한다 → 병합 때 한 번에 묶는다.

산출: `J-ESR/nonlife_ratio_census_{A,B,C}.json` · `J-ESR/nonlife_ratio_census.json` ·
`J-ESR/merge_nonlife_ratio_census.py`. 게이트 REDUCED(jp-scope) gate-clear · offline 242 passed.
...
- **실측이 PM §5 를 뒤집었다**: 이 축은 東京海上HD·MS&AD 를 잡고 **かんぽ 220 은 못 잡는다**(220% 가 원문에 실재하므로). 그래서 UH-21 신설.
- `JP_ESR_ADJUSTED_FIGURE`(UH-21): 「라벨동반 산문 조각 ≥1(0이면 기권) × 전부 한정어 × 같은 문서에 한정어 없는 대안값 존재」. 정상 14사 오탐 0, かんぽ 220 발화 / 181 미발화.
  한정어 단독 조건을 안 고른 이유: 목록이 넓어지는 경로에서 정상 3사가 거짓 발화한다(실측 표).
- 한정어 정본을 `docs/domains/claude-agent-jp.md §3` 에 등재. 정의 표지(「規制ESR」·「内部管理ベース」)는 한정어가 아니라는 함정도 같이.
- severity YELLOW + 이빨은 push 묶음(배포본 증거에 면제 없는 발화가 남으면 FAIL). 축이 죽는 유일 경로(옛 수집기로 필드 누락)는 RED 로 막았다.
- PM-2026-09-13 `closed`. 잔여 UH-22(승격 판단)·UH-23 후속(코드↔문서 대조 테스트).
...
- **왜 이제서야 병렬인가**: 서브에이전트 정의(`.claude/agents/*.md`)가 `.gitignore` 의 `.claude/` 한 줄에 걸려 저장소 밖에만 있었다. CLAUDE.md §10 은 "stage 별 모델을 티켓 유형으로" 규정하는데
  실행체가 없어 클라우드 세션이 혼자 돌고 있었다. 이 라운드에 정의 7개 + 스킬 5개를 커밋하고 ignore 를 좁혔다(머신별 `settings.local.json`·`launch.json` 만 제외).
- **검증(Opus)**: 미검증 7사 1차 원문 대조 → 6 확인 / 1 정정. 明治安田生命 **208.0 → 208.7**(統合報告書2026 분책 p10, グループESR=標準·内部 중 낮은 쪽; 標準モデル 連結 213.4% 병기; 종전 출처는 PDF 가 아닌 디렉토리 페이지였다).
- **산정기준이 섞여 있다는 사실을 데이터로 고정**: census `esr_basis` 열 신설 — 규제 표준모델 4(au·明治安田損保·日本生命·かんぽ) · 내부모델 7 · 내부관리 3(朝日·T&D·住友) · 미확인 2(ライフネット·SOMPO).
  화면은 이 값을 렌더하지 않는다. **랭킹 비교가능성 표기는 owner 판단 대기.**
- **preliminary 를 명시열로**: 키워드 추정이 양방향으로 틀렸다 — かんぽ(일본어 원문 인용이라 미검출) / 朝日·富国(notes 안 다른 수치의 속보 표기를 헤드라인의 속보로 오판). 실측 결과 4사만 true
  (ライフネット·住友·日本生命·かんぽ). 빌더는 명시열 우선, 없으면 종전 키워드 폴백.
- **목표레인지 null 3건 해소**(전부 원문 직접): 富国 230~270%(결산자료 p13) · 明治安田生命 165%以上(분책 p10) · かんぽ 150~220%(p35). 7/15 부착. (23) census 의 "상호회사 전원 미공표" 는 오판이었다.
- **dead 5건 전건 해소**: 朝日 `company/intro/disclosure/` · キャピタル損保 `company/manual/pdf/`(파일명 동일, `/pdf/` 루트 폐지) · オリックス生命 `about/results/disclosure/`(sitemap 실측: `/company/`→`/about/` 개명) ·
  日本生命 `kaisha/gyoseki/report/disclosure/ichiran/disclosure_2026.html`.
- **점검기 자체의 오탐 1건 수정**: `probe()` 가 본문 앞 400KB 만 읽어 인라인 base64 이미지에 밀린 `<a>` 를 못 보고 アクサ生命 3행을 `spa_shell` 로 찍었다 → html 이고 앵커 0개이며 절단됐을 때만 `hard_cap`(4MB)까지 더 읽는다.
- **진짜 SPA 2사 처리**: FWD生命(Next.js CSR) 은 `__NEXT_DATA__` 서버렌더 JSON 에서 연도별 disclosure PDF 14건을 꺼내 FY2025분을 `disclosure_url` 로 지정 / ヤマップ는 전 17페이지에 재무공시 페이지 자체가 없어 not_found 유지.
- **blocked 의 실체**: census 인용은 TMNF 1건뿐이고 같은 URL 이 5회 중 4회 200 — Akamai **속도제한**이다(죽음 아님). `jesr_http.RATE_LIMITED_HOSTS` 등재, 수집기는 이 호스트에 5초 간격.
- **EDINET 잔여 확정 — 원인은 전부 2026-04-01 그룹 리브랜딩**: 第一ネオ生命 = ネオファースト生命 E35324(의무 없음) · 第一アイペット損保 = アイペット損保 E33935(의무 있음) · 大樹生命 미등록(구 三井生命 포함 전수검색 0건).
  **第一生命HD 자체가 「株式会社第一ライフグループ」로 개명**(有報 S100YC7A 표지 「旧会社名：第一生命ホールディングス株式会社」, 코드 E06141 불변). `edinet_codelist.py` 에 개명 추정 표시(`rename_suspected`) 추가.
- **アクサ生命·楽天損保 가설 기각**: "결산월이 3월이 아닐 것" → 코드리스트 決算日 3月31日 동일, 15개월(결산기 2회) 스캔에 有報·訂正·半期·臨時 0건, 금융청 제출기한 연장 승인 목록에도 없음. 의무 실효 가능성(사유 미확정).
- **부수 정정**: 大樹生命 `parent_group` 朝日生命(오기) → 日本生命(2016 경영통합).
- **포스트모템 PM-2026-09-13** 신설 — false-green 메커니즘·룰 4종(오탐억제 포함)·**전부 미배선(UH-18)**·후속 티켓. README 색인/UH 표 갱신.
