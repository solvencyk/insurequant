...
**부수 발견**: ① AIA 2024.4Q 항목46 이 억원 아닌 백만원(100배)으로 마스터에 들어가 있다 — `max(base−steep,0)` 뒤로 숨어 `36_irr` 룰이 구조적으로 못 보는 자리(`inbox/parser/20260922T1200Z` 발주). ② 발행사 표가 스스로 안 맞는 2셀(BNP카디프 2023.2Q 6시나리오 0.74%·DB생명 2025.2Q 2시나리오) — 공시대로 두고 `비고` 박제.
...
| **KR0094 신한라이프** | **2026.2Q** | 〃 (owner **2026-09-01** 승인) | raw `FY2026_Q2/raw/KR0094_신한라이프생명보험.pdf` p28 순자산가치 6열 `6,319,163 / 6,448,867 / 5,427,499 / 7,105,796 / 6,348,177 / 6,219,031` · Ⅳ `1,065,545` = 마스터 item41~46·item36 정확 일치. item36 은 `item19 = sqrt(36~40·MARKET_M)` 축에서 rel −0.00099% 로 닫힘. 잔차/item36 +27.97% 로 기존 대역(+5.25~+25.62%)을 처음 넘었지만 등재 근거는 '크기' 가 아니라 위 두 조건(다른 축에서 닫힘 + raw 정확일치)의 실측이다 — **이 확장을 선례로 등재를 늘리지 말 것.** 적용후 41-46 은 p19 O/X 표 전항목 X + p21 "전·후 동일" 각주 근거 미러(`scripts/fix_20260901_kr0094_2026q2_irr_post_mirror.py`). 원인 `UNEXPLAINED`. **2026-09-21 validation**: 이 행이 코드·원장·테스트에는 있었는데 이 표에만 빠져 있어 owner 티켓 `inbox/validation/20260921T0215Z`("36_irr 이 RED 안 됨")가 났다 — 룰은 이 칸을 매 실행 보고 있다(538/538 버킷 평가, 잔차 재계산 == 박제, `_probe_20260921_36irr_census.py`). | **+2,979.8031** (+27.97%) |
...
> 근거 원장: `data/_gold/kics_exemption_provenance.json` **6건**(2026-08-21 5건 + 2026-09-01 KR0094 2026.2Q 1건), **status `VERIFIED`**(이 PDF 들은
