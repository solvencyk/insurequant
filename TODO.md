...
| F6 | **CSM 상각 schedule yearly granularity** | 현재 4 bucket (y1/y1_y3/y3_y5/y5_plus) → 연도별 10 bucket (y1..y10) + 5y aggregates (y11_15, y16_20, ...) | raw measurement extract에 1년 단위 (1년~10년) + 5년 단위 (11~15, 16~20 등) 데이터 이미 존재. (a) `scripts/viz_build_ifrs17_panels.py` `extract_amort_schedule` 확장 — yearly cols 보존 (b) JSON schema 변경: `buckets.y1..y10` + `buckets.y11_15` 등 (c) `templates/IFRS17.html` panel 2 chart config 업데이트 — bar labels 확장 (d) backward-compat: 기존 y1_y3 등도 유지 OR fully migrate. 결정 후 진행 |
