...
> Last updated: 2026-08-25 (f) (answered 3건 재확인 — CSM 워터폴이 연결로 넘어간 회귀 적발 · 불변식 1번 배선) · Stage 3/5 — validation
...
**(2026-08-25 f, answered 3건 sender 재확인) 🔴 티켓 A·B 반려(iter++) · 티켓 C 종결.
CSM_waterfall 마스터가 14 company-quarter 에서 `연결` 기준으로 넘어가 있다 — 게이트는 초록.
마스터는 한 셀도 안 고쳤다(재확인 발주). 불변식 1번은 화면 축까지 내가 배선했다.**

> 티켓 `inbox/parser/20260825T1520Z` (CSM 상각 항등식, iter 2 반려) ·
> `inbox/parser/20260825T1340Z` (FY 기초 4사, iter 2) ·
> `inbox/_resolved/20260825T1130Z` (배포본 소진율, **resolved**)
>
> ### 🔴 제일 중요한 것 — 65535 "손상 사본" 은 `연결` 주석이었다
>
> 커밋 `8a3b930` 이 삼성생명 10분기 · 신한라이프 4분기의 CSM 상각을 **별도 → 연결**로
> 옮겼다. 항등식이 1.0000 이 된 것은 워터폴을 고쳐서가 아니라 **PL 과 같은 (틀린) 기준으로
> 맞춰서**다. 파일별 grep 이 결정적이다 —
> 삼성생명 FY2024: `_00760.xml`(감사보고서=별도)에 1,369,541(수정 **전** 루트값) ·
> `_00761.xml`(연결감사보고서)에 1,393,380(수정 **후** 루트값). 신한라이프 FY2025:
> 별도에 6,969,672(커밋이 **버린** 값) 4회 · 커밋이 **고른** 6,972,351 은 별도에 **0회**.
> 두 값 차 2,679백만은 2024.4Q(7,226,793−7,224,114)와 **같은 상수** = 연결효과.
> `blocks_for_dir` 는 `_00761` 만 버리는데 **본문 XML 에 연결·별도가 둘 다 들어 있다.**
>
> **PL_breakdown 이 연결 기준이다** — 삼성생명 2024.4Q 1,393,380 · 2023.4Q 1,384,276 ·
> 신한라이프 735,862/732,881 전부 `_00761` 에만 있다. 진짜 결함은 PL 쪽이다.
>
> **삼성생명은 지금 혼합 기준**: item1/item6 은 별도(owner gold 122,474.0 앵커, raw 로
> 별도 확인) · item2~5 는 연결. 어느 공시에도 없는 조합인데 항등식은 닫힌다
> ("등식은 0들로도 닫힌다"). 기말 CSM 은 CSM 버블맵의 크기축이다.
>
> **회귀**: 티켓 B 가 Δ=0 으로 닫은 신한라이프 FY 경계가 **Δ=+26.8 로 재개방**됐고
> 2026.1Q/2Q 에 Δ=−11.6 이 **새로 생겼다**. 경계 census 는 답변이 보고한 233/18/1 이 아니라
> 현재 **231/20/1**.
>
> ### 회귀 스윕 — 다른 회사는 안 바뀌었다(그 주장은 통과)
>
> `waterfall_for_dir` read-only import 로 신·구 코드 **426 company-quarter** 전수 대조:
> 변화 **14 / 426**, 회사 2곳(삼성생명 10 · 신한라이프 4). 한화생명·현대해상 포함 35사 변화 0.
> 마스터 실변경은 `값` 126셀 + `값_당분기` 132셀 = **258**(답변의 "126" 은 `값` 만).
> 답변이 서술하지 않은 변경 2개 적발: `pick_combined_agnostic` 의
> `code == "KR0069"` 워커 우선 · `code == "KR0094"` 65535 드롭 — **회사코드 하드코딩**이고,
> 코드 주석에만 있는 더 큰 회귀 사실("blanket 적용 시 11~12 other companies 회귀")이
> 답변·커밋 메시지 어디에도 없다.
>
> ### 남은 8건 건별 재판정
>
> - **DB손해 `RESTATEMENT_BASIS` 3건 — 근거 확인.** 루트값 `311,850` 은 2023.1Q 필링에
>   없고 **2024.1Q 필링에 5회**(당기 322,368 옆 = 전기 컬럼). 2023.1Q 자신의 값 329,466
>   (= PL)은 그 필링의 연결·별도 **양쪽**에 있다(이 회사는 연결/별도 차 없음). 분류 유지 —
>   단 vintage 결정만 내리면 등재부에서 지울 수 있다.
> - **미래에셋 2025.2Q — 결론은 맞고 논증은 과장.** "누계인데 2Q만 200억 낮은 것은 불가능"
>   → 483.6<791.3<1,531.21 은 단조라 **불가능이 아니다**. 실제 근거는 증분 불균형
>   (+307.7/+739.9 vs PL +508.3/+539.1). **raw 확정 불가**: 2025.2Q 필링에 79,130·99,207
>   둘 다 0회, 빌더는 item5 를 `None` 반환. **루트값 −791.3 은 `why` 가 빈 gold override**.
> - **하나생명 2024.4Q 충돌 — 재현됨(Δ+57.2 = 4446.8−4389.6).** 단 item5 만 바꾸면 Δ는
>   −5.1 이고 +57.2 는 diag **블록 전체** 채택 시다(답변의 인과 서술 부정확). 원인은
>   diag=재작성 전 / master item1=재작성 후 → **`RESTATEMENT_BASIS` 로 재분류**할 것.
> - 교보생명 2026.1Q · 신한라이프 2026.1Q/2Q `UNRESOLVED` 는 **정직하게 적혔다**(추측 없음).
>   신한라이프 2026 2건은 연결·별도 차일 가능성이 매우 높다(2025 분기를 연결로 옮기자
>   같은 잔차가 사라졌다).
>
> ### 티켓 C — 종결. 불변식 1번을 이번에 닫았다
>
> **캡 제거 확인**: per-bond 원천에서 빌더를 안 쓰고 39사 전건 직접 재계산 → 6사 값·
> `>100%` 집합 모두 일치, 배포본↔빌더 **전 필드 diff 0**(39행/39행, 같은 분기).
>
> **불변식 1번은 iter 2 시점에 안 닫혀 있었다 — 변이시험 실측.** 배포본
> `tier1_hybrid_issued_eok` 1,000.0 → 0.0 / → 500.0 둘 다 `live_artifacts` · `data_contract`
> **양쪽 exit 0**. 화면은 "발행 0억 / 한도 694억" + 도넛 "100%+ (실제 144.1%)" 라는 자기모순을
> 그리는데 아무 룰도 안 걸렸다(항등식 분자는 `_recognized_eok`, CAPSEC 은 빌더 파일을 본다).
> → `scripts/validate_live_artifacts.py` 의 배포본↔빌더 대조를 **K-ICS.html 이 읽는 5필드
> 전부**로 확대(`_TIER_SCREEN_FIELDS`, 한쪽 결측도 RED). 재측정: M2/M3/M5/M9 전건 **exit 2**,
> 무변이 **exit 0**(오탐 0). **남는 범위 명시**: 화면 밖 3필드
> (`tier1_grandfathered_hybrid_eok` · tier2 `hybrid_eok` · `grandfathered_subordinated_eok`)는
> 여전히 통과한다. `_load_tier` 재조준(불변식 1번 본체)은 전 버킷 시뮬레이션 뒤 별건.
>
> ### 부수 적발 — gold 출처 공란 44건
>
> `data/_gold/user_csm_cells.json` 381건 중 `why`/`note` 공란 **44건**
> (KR0003 12 · KR0072 5 · KR0079 27). 출처 없는 durable override 는 다음 세션이 검증할 수
> 없다. 티켓 B 답변이 `spawn_task` 로 발주했다던 미래에셋 재조사는 **inbox 에 없어서**
> 내가 티켓으로 다시 냈다(`inbox/parser/20260825T2200Z`, 범위에 이 44건 포함).
>
> ### 내가 틀렸던 것
>
> 원 티켓 `20260825T1520Z` §① 을 쓴 것이 나다. *"루트 CSM_waterfall 이 원문과 다르다.
> **PL 쪽이 맞다**"* 가 틀렸다 — 루트가 별도로 맞았고 PL 이 연결이었다. 세 소스(diag·viz·
> history)가 PL 편이라는 것을 "루트만 틀렸다" 로 읽었는데, 그 셋이 **같은 연결 블록**을
> 보고 있었을 뿐이다. **소스 3개가 일치한다는 것은 기준이 같다는 뜻이지 옳다는 뜻이 아니다.**
