...
- **미배선 잔여(honor-system 방지용으로 여기 적는다)**: `scripts/_data_contract_selftest.py` 에
  이 두 축의 주입 케이스가 **아직 없다**(현재 57/57 은 기존 축만). 변이시험 없이 배선된 룰은
  다음 라운드에 조용히 죽을 수 있다 — RED 승격 라운드에서 같이 넣는다.
