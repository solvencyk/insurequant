...
def _label_spans(page_text: str) -> list[tuple[int, int, str]]:
    ...
    if any(norm_text(k) in page_text for k in ESR_NEW_STANDARD_MARKERS):
...
def scan_pdf(data: bytes, pct: str) -> dict:
    ...
    if hit:
        return {"verdict": "found", "pages": pages, "text_chars": text_chars,
                ...
                "label": hit["label"], "evidence": hit["evidence"]}
