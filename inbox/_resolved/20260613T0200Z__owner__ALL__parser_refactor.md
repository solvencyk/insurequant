...
## 답변 추가 (parser 2026-06-14 — kics lane: REFACTOR-3 slice2 평가 = 보류 권장)

kics 레인이 slice2를 검토. **결론: 지금 하지 않는 것을 권장(over-engineering).** owner 판단 요청.

### 근거
- **대상이 정적 config가 아니라 파라미터화 로직**: `make_quarter_column_picker._exact_quarter_match`는
  분기별로 계산되는 predicate ~12개(`f"{y_full}년{q_num}Q"`·`period_month_short in c and y_full in c`
  ·q_num==4 가드 등). slice1의 정적 regex 튜플과 달리, "config로 추출"하면 (op, template, guard) 튜플로
  로직을 데이터 인코딩 → 현재 가독성 좋은 함수보다 **오히려 복잡·불명확**해짐. `_canonicalize_table_label`도
  regex 변환 로직(이미 `_AUDIT_LABEL_ALIASES`는 slice1에서 company_handlers로 이관됨).
- **KR-keyed 아님**: parser에 `if code==KR00XX` 분기 0개(전수확인). 463개 KR-ref는 download/report 스크립트의
  정당한 config(URL·로스터). owner가 slice2를 **"진짜 KR-keyed 노브 나오면 dict dispatch"로 명시 gating** —
  그 트리거가 아직 없음.
- **이번 세션 실증**: 시장위험 36-46 회수(146 all-five·환각0)를 **회사분기 하드코딩 없이 일반규칙+config+
  reconcile 게이트**로 해결 = 레지스트리 원칙(다양성을 config로) 이미 충족. 새 변형이 코드로 안 착지함.
- CLAUDE.md "안 망가진 것 리팩터 금지 / 단일사용 추상화 금지 / 시니어가 과하다 할 코드 금지"에 배치.

### 권장
- slice2는 **genuine KR-keyed 핸들러 필요가 실제 발생할 때**(owner gate) 착수. 그때 column-picker quirk·
  값 reconcile 같은 진짜 코드-귀속 노브를 `company_handlers.REGISTRY[code]`로 흡수 = dead scaffolding 회피.
- **K-ICS 레인 REFACTOR 백로그는 고가치분 완료로 사실상 닫힘**: slice1 레지스트리(section/label/alias config화)
  + DEDUP-1/2 + GOLDEN-E2E(csm) done. slice2는 reasoned-defer.

status: open → **resolved 권장** (slice2 owner-gated defer; 이견 시 회신).

## owner 종결 (2026-06-14)

검증 완료 — `pytest tests/unit/` **110 passed** + `src/`에 `if code==KR` 분기 **0개** 확인. ifrs17 6/6 +
E2E 3종, kics slice1 + DEDUP-1/2 + E2E(csm) 디스크 반영 확인. **REFACTOR-3 slice2 보류 승인**(owner-gated;
over-engineering 근거 타당, 트리거 미발생). 파킹 항목 → `TODO_parser_kics.md`. 스레드 resolved → `_resolved/` 이동.
