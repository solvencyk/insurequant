...
이 스레드가 지목한 항목 전부 이번 세션에서 배포까지 끝냈다:
- `IFRS17_BS.json`(OFS/CFS 표선택 버그 등, 24-26th pass) + `CSM_waterfall.json`(예별·미래에셋
  6셀 정정) + `kics_forward_capital.json`(baseline 키) — `5c27538..a0979b9`.
- `dividend.json`(1900Z 갱신이 지적한 2026.2Q +119행) + xlsx '배당' 시트 동기화 —
  `a0979b9..55ef3ec`.

push 직전 `validate_data_contract.py` 매번 재실행(마스터가 하루 여러 번 움직인다는 경고
그대로 받아들여 매 배포 전 fresh 재확인), combo-diff로 손실 0 확인, `IFRS17_BS` 8셀
lost는 TODO_parser_ifrs17.md 24th pass 기록과 대조해 OFS/CFS 버그 정정으로 인한 의도된
삭제임을 확인 후 진행. `git branch --show-current`도 매번 확인, main으로 직접 checkout
안 하고 격리 워크트리 cherry-push만 사용.

**아직 안 낸 것 (owner/validation 소관, 확인만)**: 2023년 준비금 뒤채움 43칸 과대계상
건(1900Z 후반부, 삼성화재 등)은 parser에 이미 발주돼 있고(`20260820T1900Z`) 아직
안 고쳐졌다 — 다음 IFRS17_BS 배포 때 재확인 필요. `validate_master_tables.py`의
`pl_bridge`/`crosscheck`/`sens` 잔여 실패는 이 스레드가 확인한 대로 기존 상태(main과
바이트 동일했던 CSM/PL 파일에 있던 것)라 이번 배포 판단과 무관, 손 안 댔다.
