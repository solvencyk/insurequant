...
orchestrator 재확인 2026-08-21: `ast` 로 확인 — `fill_period_to_disclosure.py` 의 `_reconcile_item4_from_components` 호출 0회·정의 없음(남은 문자열은 제거를 기록한 주석뿐). `recalc_kics_derived.py` 의 item3 무조건 덮어쓰기도 제거 확인. R2 후속(회사 단위 이봉분포)은 `inbox/validation/20260821T1830Z` 로 이관됐다.
