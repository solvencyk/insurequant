...
## 답변 추가 (parser-kics 2026-06-16 — §C + §A 경로정정 완료)

**§C `docs/domains/claude-agent-kics.md` (owner 결정=stub 동결+포인터) — ✅ DONE:**
- 상단 **포인터 헤더**: "운영 정본 = `.claude/skills/kics-parser/` SKILL + `kics-json-validation-rules.md`; 본 문서 =
  2026-05 설계 stub(동결); 시장위험 36-40/IRR 41-46/금리민감도는 SKILL·rules 참조; 충돌 시 코드+SKILL 우선."
- 오기 정정: `kics_data.json`→`kics_disclosure.json`(L3); flow 링크 pre-reorg 3건 → `docs/flows/*`(L6/L11/L16).
- **§3 push 정책**: "push 완료 전 세션종료 금지" → "검증 보고 후 push=**owner-gated**, parser self-approve 안 함(TODO #7)".
- 37社 카운트는 헤더 stub 경고로 갈음(강제 정정 안 함).

**§A 출력경로 정정(ifrs17이 kics에 남긴 건) — ✅ DONE:** §2.1 L61 `md_inbox/<KR>/<period>.md`(역순) →
`run_harness --stage parse → data/disclosure/FY*/parsed/*.md + md_inbox/FY*_Q?/<KR>_<name>.md → fill_* → row dict`.

(전 편집 UTF-8 no BOM read-back OK.) status: **A·B·C 전부 done** — 본 건 종결.
