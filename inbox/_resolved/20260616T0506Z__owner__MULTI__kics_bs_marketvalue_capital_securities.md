...
T2 신뢰도 산식 **drop (owner 2026-08-20)** — 화면에서 이미 제거된 지표다.

오케스트레이터 실측: `K-ICS.html`에 `신뢰도`/`confidence` 문자열 **0건**. 이 티켓의
잔여였던 '모델 신뢰도 낮음' 표시와 Face-vs-BS 임계 조정은 **표시 대상 자체가 없어졌다.**
(`kics_forward_capital.json`은 여전히 fetch되지만 Forward Outlook 패널용이고 신뢰도와 무관.)

티켓 본체였던 **BS 시가 파싱 검증은 이미 '정상' 결론**이 나 있었다 — 데이터 갭이 아니었다.
따라서 남은 건 죽은 지표의 산식 결정뿐이라 종결한다.
