...
## 답변 (parser/kics 2026-06-16 — K-ICS 도메인 SKILL 결정화 완료)

**산출물:** `.claude/skills/kics-parser/` (Anthropic `skill-creator`로 작성, ifrs17-parser 형제 구조 미러링)
- `SKILL.md` (124행) — 트리거 description(K-ICS 파싱·재추출·새분기 온보딩·게이트 디버그·지급여력 수치질문 시
  자동 로드 + **IFRS17/CSM/DART 레인 명시적 제외**) + 운영 코어(2-lane split·pipeline at a glance·8대 trap·작업법).
- `references/pipeline-map.md` (110행) — raw→docling→fill_period(1-28)→fill_subitems(29-35)→fill_market(36-40/
  41-46)→gate 배선, 항목 1-46 번호표, kics_disclosure/rate_sensitivity 스키마, 게이트 룰 요약(1-10·8_life·19_market·
  36_irr + 4대 구조 게이트 census/parent-zero/breakdown-presence/tooling-fail), run/verify 레시피.
- `references/quirks-and-traps.md` (90행) — even/odd cadence·parent-zero·단위(백만원↔억)·△부호·자본tiering 한도·
  사별 quirk 표(AIA/코리안리/서울보증/예별MG/내부모형사/카카오/삼성·신한 split-table/이미지사)·reconciliation 항등식·
  검증 5대 사각·Windows/인코딩.

**SOT vs SKILL 정본 (owner 요구 — 1줄 명시):** **rules SOT = `docs/agents/kics-json-validation-rules.md` +
`docs/agents/kics-market-risk-decomposition.md` + 코드 `src/solvency/validation/kics_json_rules.py`(최종)**. SKILL =
그 위 **운영 트리거 레이어**(현행 pipeline map + traps). 내용 복붙 없이 요약+참조.

⚠️ **`docs/domains/claude-agent-kics.md`는 얇고 부분적으로 stale** — 결정화 중 확인: 38행뿐이고 **시장위험 36-40·
금리위험 IRR 41-46·금리민감도가 전부 누락**(bootstrap stub), 출력명을 옛 `kics_data.json`으로 칭함, flow 링크가
pre-reorg. durable한 건 추출타깃 목록·생명장기 6 sub-risk·cross-quarter integrity rule·삼성/신한 split-table뿐.
SKILL은 market/IRR/민감도를 **코드+validation-rules 기준**으로 작성했고, 그 doc 의존 시 stale 주의를 SKILL에 박아둠.
→ **owner 판단 요청(별건)**: claude-agent-kics.md를 현행 코드 기준으로 리프레시할지(아니면 SKILL이 사실상 운영정본
역할이므로 doc은 설계 stub로 동결할지).

**위치 주의:** `.claude/`는 `.gitignore`(L80)라 SKILL은 **머신-로컬**(push 안 됨) — ifrs17과 동일. 공유 정본은
tracked `docs/agents/*.md`(rules SOT), SKILL은 이 checkout의 kics 세션이 자동 로드하는 로컬 편의 레이어.

**description 튜닝:** 수기로 트리거 강화(파싱/재추출/새분기/게이트 디버그/시장위험 회수/지급여력 수치질문 +
**IFRS17 네거티브 경계**). 경험적 최적화(`skill-creator` run_loop.py = `claude -p` 5-iter)는 ifrs17 레인과 동일하게
보류 — owner 원하면 실행. (검증: 3파일 UTF-8 no BOM read-back OK, frontmatter 유효, SKILL.md 124행 < 500 권장.)

status: kics SKILL done (machine-local, gitignored). 양 레인 모두 결정화 완료. claude-agent-kics.md 리프레시는 owner 판단 대기.
