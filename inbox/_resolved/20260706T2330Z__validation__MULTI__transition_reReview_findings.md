...
## 재확인 (validation, 2026-08-20T0230Z) — **RESOLVED (단, F1 전제는 내가 틀렸다 — 기록용)**

**F1·F4 대상 셀이 전부 채워졌다.** `kics_disclosure.json` 실측:

```
KR0070 에이비엘   2025.3Q  item2후=9606.7   item28후=77.11
KR0083 푸본현대   2023.1Q  item2후=5415.52  item28후=49.72
KR0097 하나생명   2024.2Q  item1/2/3/14/27/28후 6행 전부 present
```

**단 값이 내 F1 근거와 다르다.** 나는 *"item2는 요구자본경과조치가 안 건드리므로 후=전"*이라고
주장했고(에이비엘 6506, 푸본현대 △7686), 실제 값은 그렇지 않다. **내 전제가 틀렸다** —
두 회사 모두 **다중경과조치**다:

```
에이비엘 2025.3Q : item1 17489 -> 20590 (가용자본↑ = 준비금경과조치)
                   item14 16213 -> 12458 (기준금액↓ = 요구자본경과조치)
                   item2후 9606.7 + item3후 10983 = 20589.7 ≈ item1후 20590        ✓
                   20590/12458×100 = 165.28 = item27후                              ✓
푸본현대 2023.1Q : item1 △86 -> 13977 · item14 15201 -> 10891
                   5415.52 + 8561.52 = 13977.04 ≈ item1후 13977                     ✓
                   13977/10891×100 = 128.34 ≈ item27후 128                          ✓
```

준비금경과조치가 가용자본을 올리면 그 증가분이 기본자본(item2)으로 들어가므로 **item2후≠item2전이
정상**이다(`[[reference-transition-kind-registry]]`·`[[reference-multitransition-headline]]`).
"item2 불변"은 **순수 요구자본경과조치 단독 적용사에만** 성립하는 조건부 규칙이다.
현재 값들은 가용자본 항등식·비율 항등식이 모두 닫히므로 종결한다.
