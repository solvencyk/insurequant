...
## 재검증 (validation 2026-07-16) — 통과, 잔여 2건 owner escalate

**fill 정당성 적대검증 = PASS (V17 가짜복사 아님 확인):**
- 삼성생명·동양생명·**한화생명**(=전 미러링분) 전부 **공통경과조치사 정당**: item1(가용자본) 후는 2025.2Q에
  실효과(삼성/한화 등 Δ+825·동양 Δ+1188)로 ≠전인데, **item15~21(요구자본) 후=전(±1억 반올림뿐)** →
  공통(TFI)은 가용자본만 건드리고 요구자본 불변이라는 도메인과 정합. 미러링 = 정답.
- **무회귀**: `_transition_ratio_after_capture`(COPY/LOWER/AMT_MISMATCH)=0, mmult=0, 항등식=0,
  분산효과음수=0. 산술 후검사 전부 유지. continuity break **117→62셀**(display push-block **47→4**).

**하나생명 item17후=1757.32 출처(질문 회신)**: round3 이전부터 json에 존재, fill 스크립트에 명시적
근거 없음 — **당신 추정(item29-35 부분채움 partial-mmult 아티팩트)이 가장 유력**(1757.32는 clean
phase-in도 R7 full-mmult도 아님). validation 측 authoritative fill 이력 없음. page 2001.90이 확인되면
그 값 채택 권장(단 이 셀은 `_AFTER_SUBRISK_NOT_DISCLOSED` 등재분이라 mmult후 검사 skip = 비차단).

**잔여 2건 = 파서 추가작업 아님, owner escalate (raw 도출불가 확정):**
- **흥국생명 2024.4Q [15,16,22]**: image PDF + TIR+TER 다중경과 R4 재현불가(disjoint 17/19 역산 item15
  14,747 vs 헤드라인 16,987, Δ2,240 비반올림) — 당신 판단대로 결합불명. → owner `_POST_PARENT_NOT_DISCLOSED`.
- **하나생명 2024.4Q [16]**: item16후는 산술 파생 가능(Σ(17~21)후−15후 = 1369.09)이나 입력 item17후가
  위 불확실성 보유 → 파생값도 불신. 이미 비표준(감사보고서 재무상태표) 공시 등재분. → 같이 owner escalate
  (owner가 "item16 파생 채움" vs "부모후 exemption" 택일).

두 건 TODO_validation.md V18 + changelog 기록, owner 큐로 올림. **파서 이 티켓 추가작업 없음 → resolved.**
(non-display 잔여: 코리안리·악사·처브·IBK 2023.2Q·하나손/하나생 2023.2Q = push 비차단 워크리스트,
당신이 ticket대로 non-display 미착수한 것 존중.)
