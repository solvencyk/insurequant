"""Skim-only extractor for section 14(3)(4) reinsurance rollforward (A4)."""
...
@dataclass
class ExtractedReinsuranceTable:
    caption: str
...
def _classify_kind(caption: str, header_hits: int, row_labels: list[str]) -> str:
    joined = caption + " ".join(row_labels[:8])
    if header_hits >= 2 or any(k in joined for k in ("\uCE21\uC815\uC694\uC18C", "\uAD6C\uC131\uC694\uC18C")):
        return "measurement_rollforward"
    if any(k in joined for k in ("\uBCC0\uB3D9\uB0B4\uC5ED", "\uAE30\uCD08", "\uAE30\uB9D0")):
        return "asset_liability_rollforward"
    return "unknown"


def _has_reins_structure(t: ExtractedReinsuranceTable) -> bool:
    joined = " ".join(_first_col_labels(t.rows))
    if sum(1 for s in _ROW_STUBS_STRONG if s in joined) >= 1:
        return True
    return t.block_type in ("reinsurance", "mixed")


def is_mvp_table(t: ExtractedReinsuranceTable) -> bool:
    """MVP A4: ceded reinsurance rollforward for target slice."""
    if not _has_reins_structure(t):
        return False
    if t.block_type not in ("reinsurance", "mixed"):
        return False
    sub = _caption_subsection(t.caption or "")
    if _is_reinsurance_subsection(sub) is False and t.block_type == "direct":
        return False
    ...
    return t.slice_label == "long_term"
...
def _score_table(t, company_name: str) -> tuple[int, str, str, str, str, list[str]]:
    ...
    if primary_hits >= 2:
        score += 4
        reasons.append(f"caption: reinsurance keywords x{primary_hits}")
    elif primary_hits == 1:
        score += 2
        reasons.append("caption: 1 reinsurance keyword")
    ...
    if strong_hits >= 2:
        score += 3
        reasons.append(f"rows: reinsurance stubs x{strong_hits}")
    elif strong_hits == 1:
        score += 2
        reasons.append("rows: 1 reinsurance stub")
    elif weak_hits >= 2:
        score += 1
        reasons.append(f"rows: weak stubs x{weak_hits}")

    block_type = _classify_block(caption, row_labels)
    if block_type == "direct" and primary_hits == 0:
        score -= 2
        reasons.append("penalty: direct-only block")
    elif block_type in ("reinsurance", "mixed"):
        score += 1
    ...
    table_kind = _classify_kind(caption, header_hits, row_labels)
    ...
    if primary_hits == 0 and strong_hits == 0:
        if score > 3:
            score = 3
            reasons.append("gate: weak reinsurance signals capped at 3")

    return score, block_type, slice_label, slice_policy, table_kind, reasons


def extract_reinsurance_tables(
    xml_path: Path,
    company_name: str = "",
    min_score: int = 5,
    mvp_only: bool = False,
) -> list[ExtractedReinsuranceTable]:
    out: list[ExtractedReinsuranceTable] = []
...
def to_jsonable(t: ExtractedReinsuranceTable) -> dict:
    return {
        "caption": t.caption,
