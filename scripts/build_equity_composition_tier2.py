...
# 2026-08-31 추가분(owner 2025.4Q 결측 조사, 11개사): IBK연금보험(211/212의 "인식" 계열,
# "-공정가치측정" 없이 "인식"만 쓴다) · 신한이지손해보험(212의 "유가증권" 계열, 개념어가
# "금융자산"이 아니라 "유가증권") · AIG손해보험/신한이지손해보험(220의 "배당요소가없는"/
# "무배당" 계열 -- IFRS17 무배당 보험계약부채 단독 표기, 두 회사 다 유배당 계약이 없어
# 이 한 줄이 곧 총계다. co-occurrence 확인: 두 회사 다 "배당요소가있는"/"유배당" 형제 행이
# 없다 -- 있는 회사가 나오면 합산 로직이 필요하나 아직 관측 0건).
_BS_DETAIL_LABELS = {
    210: ("현금및현금성자산", "현금및예치금", "현금및상각후원가측정예치금"),
    211: ("당기손익-공정가치측정금융자산", "당기손익공정가치측정금융자산",
          "당기손익인식-공정가치측정금융자산", "당기손익인식금융자산"),
    212: ("기타포괄손익-공정가치측정금융자산", "기타포괄손익공정가치측정금융자산",
          "기타포괄손익인식-공정가치측정금융자산", "기타포괄손익인식금융자산",
          "기타포괄손익공정가치측정유가증권"),
    214: ("재보험계약자산",),
    215: ("유형자산",),
    220: ("보험계약부채", "배당요소가없는보험계약부채", "무배당보험계약부채"),
    221: ("재보험계약부채",),
    222: ("투자계약부채",),
    223: ("차입부채",),
    224: ("기타부채",),
...
# 2026-08-31 추가분: "상각후원가측정대출채권및기타수취채권"(대출채권+기타수취채권을 한 행에
# 합쳐 내는 회사 -- 에이아이에이생명·비엔피파리바카디프생명, 기존 "...및수취채권"과 다른
# 글자) · "상각후원가유가증권"/"상각후원가대출채권및기타금융자산"(IBK연금보험, "측정"
# 생략형 -- 이 회사는 두 행이 형제로 같이 나와 합산이 필요해 부모 라벨은 없다).
_AC_CHILD_LABELS = ("상각후원가측정대출채권", "상각후원가측정유가증권",
                    "상각후원가측정기타금융자산", "상각후원가측정대출채권및수취채권",
                    "상각후원가측정기타수취채권", "상각후원가측정예치금",
                    "상각후원가측정대출채권및기타수취채권",
                    "상각후원가유가증권", "상각후원가대출채권및기타금융자산")
...
def _bs_label(raw: str) -> str:
    ...
    거의 모든 행이 후자라 AOCI 가 통째로 안 잡혔다 -- 2026-08-20 실측). AIG손해보험은 각주를
    괄호가 아니라 꺾쇠(`<주석13,33,35>`)로 낸다(2026-08-31 실측) -- 괄호 정규식만 있으면 이
    회사는 거의 모든 세부 항목(현금/금융자산/AOCI/재보험자산/유형자산/자본금/이익잉여금)이
    라벨 불일치로 통째로 안 잡힌다(총계 3행만 각주가 없어 그것만 살아남았다)."""
    lab = _strip(raw)
    lab = re.sub(r"^[IVXⅠ-Ⅹ]+[.\s]*", "", lab)
    lab = lab.lstrip("ⅠⅡⅢⅣⅤⅥⅦⅧⅨⅩ.0123456789()가나다라마바사")
    lab = re.sub(r"\(주[석]?[\d,\s]*\)$", "", lab)
...
def _pick_bs_table(tables, markers, raw_lines):
    ...
    2026-08-31 시도 기록: 첫 행 라벨이 "자산"이 아니어도 자산총계/부채총계/자본총계가 다
    있고 항등식이 닫히면 후보로 받는 완화를 시도했다가(비엔피파리바카디프생명 -- 이 회사만
    BS 표에 섹션헤더 행이 없다) **되돌렸다**: 메리츠화재해상보험 2023.1Q 등 여러 Tier-1
    회사에서 전혀 무관한 표(그 회사 필링 안의 "(2) 최대주주(법인 또는 단체)의 최근 결산기
    재무현황" -- 최대주주라는 다른 법인의 미니 재무상태표. 우연히 자산총계/부채총계/자본총계
    라벨이 있고 닫혀서 후보가 됐고, basis 순위(OFS < CFS) 타이브레이크에서 그 회사 자신의
    진짜 연결 BS보다 앞섰다)를 집어 core 총계 셀이 대량으로 사라지거나 다른 값으로
    바뀌었다(회귀 실측: 188셀 결측 + 54셀 변경, 전부 2023.1Q/2Q -- 이 분기는 FS-API가 전사
    무응답이라 이 표 선택 하나에 core 총계 전부가 걸려 있다). "총계 3개 + 항등식 닫힘"은
    같은 회사의 BS가 아닌 표(최대주주·자회사·특수관계자 요약 등)에서도 흔히 성립해 안전한
    판별 기준이 아니었다. 비엔피파리바카디프는 셀 단위 오버라이드(`bs_manual_overrides.json`)
    로 대신 처리한다(회사 고유 1회성 표기, 15개사 census로 확인 -- 공용 표 선택 로직은
    안 건드리는 게 맞다)."""
