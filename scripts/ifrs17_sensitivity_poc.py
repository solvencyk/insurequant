# -*- coding: utf-8 -*-
"""Skim 5 PoC companies annual filings for B5 sensitivity rollforward."""
...
def main():
    ...
    for dirname, company_name in POC_DIRS:
        ...
        for xml in sorted(d.glob("*.xml")):
            tables = extract_sensitivity_tables(xml, company_name=company_name)
            per_xml.append({
                "xml": xml.name,
                "tables": len(tables),
        ...
        out = settings.extracted_dir / f"{dirname}_sensitivity.json"
    ...
    out_summary = settings.extracted_dir / "_sensitivity_poc_summary.json"
