# -*- coding: utf-8 -*-
"""Batch A1 measurement rollforward on cached annual filings (23 operational insurers)."""
...
SUMMARY_OUT = settings.extracted_dir / "_batch_measurement_summary.json"
...
def main():
    ...
    for i, row in enumerate(targets, 1):
        ...
        for xml in sorted(raw_dir.glob("*.xml")):
            try:
                tables = extract_measurement_tables(xml, company_name=kics)
        ...
        out_json = settings.extracted_dir / f"{canonical}_{rcept_no}_measurement.json"
        out_json.write_text(json.dumps(all_tables, ensure_ascii=False, indent=2), encoding="utf-8")
        mvp_json = settings.extracted_dir / f"{canonical}_{rcept_no}_measurement_mvp.json"
        mvp_json.write_text(json.dumps(mvp_tables, ensure_ascii=False, indent=2), encoding="utf-8")
        rec = {
            "kics_name": kics,
            "canonical": canonical,
            "rcept_no": rcept_no,
            "status": "ok" if all_tables else "no_measurement_table",
