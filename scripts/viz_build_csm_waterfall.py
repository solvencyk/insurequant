...
STAGE_PATTERNS: dict[str, list[str]] = {
    ...
    "amortization": [
        ...
        # 위 주석이 가리키는 그 OLD 라벨 자체가 패턴에 없었다 (2026-08-30, inbox/parser/
        # 20260830T0200Z) — 미래에셋생명(KR0079) 상품별 WIDE 측정요소표(2025.2Q/2025.3Q/
        # 2026.1Q, find_product_segmented_csm_cols 경로)의 실제 행 라벨은 "보험수익, 서비스의
        # 이전으로 당기손익에 인식한 보험계약마진"(선행 "보험수익, " 접두 + 조사 "손익에", not
        # "손익으로") — 기존 "당기손익으로 인식한 보험계약마진" 패턴과 조사 하나 차이로 substring
        # 불일치했다. None → build_csm_waterfall_master.py의 잔차식(`... or 0`)이 항목5를
        # 항목4에 통째로 흡수. 71st pass(9a067dd, 부모헤더 폴백/줄바꿈공백)가 고친 두 변형과는
        # 다른 세 번째 변형. 전사 스윕(SONBO 전체, FY2022_Q4~FY2026_Q2) 결과 이 WIDE 포맷을 쓰는
        # 3사(KR0069/KR0079/KR0087) 중 KR0069·KR0087은 각자 다른(이미 매칭되는) 라벨을 쓰고
        # 실사용 후보 블록에 영향 없음 — 실질 영향은 KR0079 전용.
