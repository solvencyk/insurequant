# -*- coding: utf-8 -*-
"""Skim 5 PoC companies annual filings for A3 insurance P&L rollforward."""
...
def main():
    ...
    for dirname, company_name in POC_DIRS:
        ...
        for xml in sorted(d.glob("*.xml")):
            tables = extract_insurance_pl_tables(xml, company_name=company_name)
            per_xml.append({
                "xml": xml.name,
                "tables": len(tables),
        ...
        out = settings.extracted_dir / f"{dirname}_insurance_pl.json"
    ...
    out_summary = settings.extracted_dir / "_insurance_pl_poc_summary.json"
