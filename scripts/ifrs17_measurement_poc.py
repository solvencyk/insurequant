# -*- coding: utf-8 -*-
"""Skim 5 PoC companies annual filings for A1 measurement rollforward."""
...
def main():
    ...
    for dirname, company_name in POC_DIRS:
        ...
        for xml in sorted(d.glob("*.xml")):
            tables = extract_measurement_tables(xml, company_name=company_name)
            per_xml.append({
                "xml": xml.name,
                "tables": len(tables),
        ...
        out = settings.extracted_dir / f"{dirname}_measurement.json"
    ...
    out_summary = settings.extracted_dir / "_measurement_poc_summary.json"
