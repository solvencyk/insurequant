# -*- coding: utf-8 -*-
"""Batch B1 BS snapshot rollforward on cached annual filings (23 operational insurers)."""
...
from src.ifrs17.bs_snapshot_extractor import extract_bs_snapshot_tables, to_jsonable
from src.ifrs17.universe import is_excluded

SUMMARY_IN = settings.extracted_dir / "_batch_all_summary.json"
SUMMARY_OUT = settings.extracted_dir / "_batch_bs_snapshot_summary.json"
...
def main():
    ...
    for i, row in enumerate(targets, 1):
        ...
        for xml in sorted(raw_dir.glob("*.xml")):
            try:
                tables = extract_bs_snapshot_tables(xml, company_name=kics)
        ...
        out_json = settings.extracted_dir / f"{canonical}_{rcept_no}_bs_snapshot.json"
        out_json.write_text(json.dumps(all_tables, ensure_ascii=False, indent=2), encoding="utf-8")
        mvp_json = settings.extracted_dir / f"{canonical}_{rcept_no}_bs_snapshot_mvp.json"
