...
_TIER2_ISSUER_INCONSISTENT: dict[tuple[str, str], dict] = {
    ...
    #
    # ── 2026-09-01 재검토 (owner 발주 `inbox/parser/20260901T0418Z`, 결론: **현행 유지**) ──
    # owner 가 기사(fins.co.kr #109853)의 롯데손보 26.1Q 기본자본 **-3,509억**이 마스터
    # item2(-3,962억)와 달라 재조사를 발주했다. 두 가지가 새로 확정됐다:
    #
    #  (a) **기사 숫자의 정체** — 기사값은 공시된 값이 아니라 `item2 + TFI 델타`의 재계산치다:
    #      -3,962 + 453.70 = **-3,508.30** (기사 -3,509억과 반올림 오차 내 일치).
    #      453.70억 = TFI 표의 (기발행 신종자본증권) 45,370백만원 = 적용후-적용전 델타
    #      ((-342,144) - (-387,514) = +45,370). 다음에 또 "기사와 우리 값이 다르다" 로
    #      시작하지 않도록 여기 적어 둔다 — **기사가 틀린 것도, 우리가 틀린 것도 아니다.**
    #
    #  (b) **item53(45,370)은 26.1Q 에도 유효** — 스테일 의심이 있었으나 근거 셋으로 기각:
    #      ① 롯데 신종자본증권 2건의 콜 날짜가 2026-12-17·2026-12-29 라 1분기에 상환될 게 없다
    #      ② 그 사이 신규 발행 기록 없음 ③ **item53 이 2026.2Q 에도 453.70**(별개 공시본에서
    #      따로 파싱된 값). `data/bonds/capital_securities_fy2026h1.json` 은 롯데가
    #      `as_of: 2025-12-31` 이라 독립 확인 불가(DART 미공시 = CAPSEC_NUMERATOR_ASOF_MISMATCH).
    #
    # **왜 고치지 않았나 — 실제로 옮겨 보고 되돌렸다(2026-09-01).** item50/51/52 를 헤드라인
    # (item1/2/3) + 델타로 갈아끼우면 이 핀 4개가 `TIER2_EXEMPTION_INPUT_DRIFT` 로 즉시 터지고,
    # 더 중요하게는 **item47/48/49 도 같은 25.4Q 스택이라 함께 옮겨야 하는데 그게 불가능하다**:
    # item47(보완자본 한도 적용 전)·item49(해약환급금 초과분)는 헤드라인에서 유도할 수 없고,
    # item48 은 `item14 x 50%` 로 계산은 되지만 **검산식이지 공시값이 아니다**(채우면 그 축의
    # 검산이 동어반복이 된다). 여섯 칸만 옮기면 `min(47,48)+49 = 29,933.6` vs `item51 = 30,918`
    # 로 **새 잔차 984.36 이 두 룰에 생긴다** — 지금은 스테일 스택이 저희끼리 닫혀 있어서
    # 그 잔차가 없다. 즉 부분 정정은 "50/51/52 는 26.1Q, 47/48/49 는 25.4Q" 라는 혼합 상태를
    # 만들고, 그건 현행보다 나쁘다. owner 확인 후 **현행 유지**로 종결.
    # 화면 영향 없음: item50~52 는 2026.1Q 에 대해 어떤 패널도 읽지 않는다(자본성증권 소진율
    # 마스터는 2026.2Q 단일 분기).
    ("KR0003", "2026.1Q"): {
