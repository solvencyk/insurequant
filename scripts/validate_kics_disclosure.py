...
_TIER2_ISSUER_INCONSISTENT: dict[tuple[str, str], dict] = {
    ...
    # --- 롯데손해 KR0003 2026.1Q ---------------------------------------------
    # **2026-09-03: 원문이 정정본으로 교체돼 이 버킷은 해제됐다. 아래는 이력이다 — 새
    # 세션이 "왜 여기 KR0003 2026.1Q 만 없지"로 헤매지 않도록 남겨 둔다.**
    #
    # ── 구본 기준 서술 (2026-08-24 ~ 2026-09-01, 지금은 사실이 아니다) ──
    # 당시 정본이던 `[롯데손해보험]2026.1Q 경영공시자료_최종 제출_20260528.pdf`(886,240
    # bytes, zip 기록시각 2026-05-29 16:41 — 지금은
    # `data/_archive/20260903T015112Z/disclosure_superseded_20260528/`)에서는
    # **발행사가 전기(2025.4Q) TFI 표를 그대로 재게시했다**는 결론이 실제로 맞았다:
    #   p21 헤드라인 당분기(2026.1Q) : 26,955 / (3,962) / 30,918 / 기준금액 20,432
    #   p21 헤드라인 직전분기(2025.4Q): 26,058 / (3,875) / 29,934 / 기준금액 20,671
    #   p22 TFI 표(구본)             : 2,605,850 / (387,514) / 2,993,363 / 2,067,069
    #                                 = 26,058.50 / -3,875.14 / 29,933.63 / 20,670.69
    # TFI 표가 직전분기 열과 소수점까지 일치했다 — 그래서 이 버킷의 RED 5건이 전부 한 원인
    # (보완자본 구성 · tier 분할 전후 · `TIER2_LIMIT_STALE` 전후)으로 면제됐었다.
    #
    # 2026-09-01 owner 재검토(`inbox/parser/20260901T0418Z`, 결론 **현행 유지**)도 이
    # 구본을 전제로 했다 — 기사(fins.co.kr #109853)의 기본자본 -3,509억이 "item2(-3,962) +
    # TFI 델타 453.70"의 재계산치임을 확인하고, item50/51/52 를 헤드라인+델타로 부분 정정하면
    # item47/48/49(헤드라인에서 유도 불가) 와 축이 갈라져 새 잔차가 생긴다는 이유로 옮기지
    # 않기로 했다. **그 전제(구본이 정본)가 지금은 없다** — 아래를 보라.
    #
    # ── 2026-09-03 원문 교체 (오케스트레이터) 이후 ──
    # `data/disclosure/FY2026_Q1/raw/KR0003_롯데손해보험.pdf` 가
    # `[롯데손해보험]2026.1Q 경영공시자료_재제출.pdf`(1,927,066 bytes, zip 기록시각
    # 2026-06-02 17:09 — 구본보다 4일 늦다)로 교체됐다. **재제출본은 TFI 표 오류가 고쳐져
    # 있다** — raw p22 "(1) 공통적용 경과조치 관련" 표(fitz 직접 판독 + docling MD 재변환,
    # 둘 다 일치):
    #   지급여력비율 131.93 / 131.93 · 지급여력금액 2,695,532 / 2,695,532 (=item1 26,955 와
    #   소수점까지 일치, 더 이상 직전분기 값이 아니다) · 기본자본 (396,229) / (350,858) ·
    #   보완자본 3,091,761 / 3,046,391 · 보완자본 한도 적용 전 824,718 / 469,814 ·
    #   보완자본 한도 1,021,614 / 1,021,614 · 해약환급금 초과분 2,267,042 / 2,267,042.
    # 재파싱해 item47~52 를 새 값으로 갱신했다(`scripts/fix_20260903_lotte_kr0003_2026q1_
    # tfi_resubmission.py`, item53/54 는 신·구 값이 우연히 같아 무변경). 이 버킷과
    # `data/_gold/kics_exemption_provenance.json` 의 대응 원장 기록을 함께 제거했다 — 원인이
    # 소멸했으므로 면제도 없앤다(한쪽만 지우면 `EXEMPTION_PROVENANCE_MISSING` 이 뜬다).
    #
    # **재검산 결과는 TODO_parser_kics.md 2026-09-03 항목 참조** — 적용전 축은 새 값으로
    # 자체적으로 닫히는지, 적용후 축에 잔차가 남는지를 게이트 재실행으로 직접 확인했고
    # 그 실측을 여기 다시 옮기지 않는다(코드에 박제된 잔차와 TODO 서술이 어긋나면 TODO 가
    # 진 사실을 놓친 것이다 — 이 게이트가 그 사고를 계속 스스로 잡는다).
    # =====================================================================
    # 아래 3버킷은 **사유가 다르다** — 코리안리·롯데(위 버킷들)는 '두 표가 서로 다른 값을
    ...
    # --- AIG손해 KR0029 2025.3Q ---------------------------------------------
    # **발행사가 재분류를 기본자본에는 반영하고 재분류 항목란은 비웠다.**
    # p14 다리표: item2(기본자본)=6,304 는 item4(순자산)=6,362 에서 이미 58.70 만큼 깎여
    # 인쇄돼 있는데, 같은 페이지의 `Ⅲ.보완자본으로 재분류하는 항목`(item13)은 **진짜 숫자
    # 0** 으로 인쇄돼 그 차감의 근거를 담지 않는다(`Ⅱ.불인정하는 항목` item12 도 0).
    # 대시도 공백도 아니다 — fitz word-bbox 좌표(item12 y=290.9 · item13 y=318.7, x=344)와
    # 240dpi 크롭 렌더링 육안 확인 2방법으로 대조했고, 그 표의 item1~14 전 항목이 마스터와
    # 소수점까지 일치해 추출 오류가 아님을 확정했다(2026-09-01, owner 옵션 (c) 지시).
    #
    # 58.70 의 출처는 p15 TFI표의 item49(해약환급금 부족분 초과분)다. **그 표는 자기 안에서
    # 완전히 닫힌다** — 같은 58.70 을 보는 다른 다섯 축이 전부 GREEN 이다:
    #   3_tier2_composition(item3=59 ≈ min(47,48)+49=58.7) · 51_tfi_tier2_composition(58.7=58.7)
    #   50_tfi_tier_split(6362.26 ≈ 6362) · 47_tier2_census · 48_tier2_limit
    # 깨지는 것은 `item4 − item12 − item13` 으로 item2 를 재현하는 이 축 하나뿐이다.
    #
    # 같은 회사 2026.2Q 는 **행 구성이 100% 동일한 표**에서 item12=300 · item13=754 로
    # 채워 공시해 다리가 잔차 −1 로 닫힌다(item49=754.39). 즉 표 구조나 파싱 문제가 아니라
    # **분기별 기재 관행 차이**다.
    #
    # 해제조건: item2/4/12/13 중 한 칸이라도 움직이면 `..._INPUT_DRIFT` RED 로 되살아난다.
    ("KR0029", "2025.3Q"): {
        "cells": {2: {"값": 6304.0}, 4: {"값": 6362.0}, 12: {"값": 0.0}, 13: {"값": 0.0},
                  3: {"값": 59.0}, 47: {"값": 0.0}, 48: {"값": 1277.76}, 49: {"값": 58.7},
                  50: {"값": 6303.56}, 51: {"값": 58.7}},
        "findings": {
