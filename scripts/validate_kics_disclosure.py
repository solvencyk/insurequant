...
_TIER2_ISSUER_INCONSISTENT: dict[tuple[str, str], dict] = {
    ...
    ("KR0075", "2024.3Q"): {
        ...
        "findings": {
            ...
            "3_tier2_composition": {"flag": "COMPOSITION_NEITHER", "residual": -220.98},
            "47_tier2_census": {"flag": "TIER2_DUPLICATE_ROW", "residual": None},
            "47_tier2_census_post": {"flag": "TIER2_DUPLICATE_ROW", "residual": None},
            "51_tfi_tier2_composition": {"flag": "TFI_COMPOSITION_NEITHER", "residual": -221.31},
        },
    },
    ...
    # `item47 == item48` 은 두 셀을 잘못 읽은 지문이 아니다 — 채무성 자본이 0인 회사가
    # 한도-체크 두 줄에 같은 숫자를 실제로 인쇄한다(좌표읽기·pixmap·전체덤프 3방법 확인).
    ...
    ("KR0075", "2024.4Q"): {
        ...
        "findings": {
            ...
            "3_tier2_composition": {"flag": "COMPOSITION_NEITHER", "residual": -242.27},
            "47_tier2_census": {"flag": "TIER2_DUPLICATE_ROW", "residual": None},
            "47_tier2_census_post": {"flag": "TIER2_DUPLICATE_ROW", "residual": None},
            "51_tfi_tier2_composition": {"flag": "TFI_COMPOSITION_NEITHER", "residual": -242.74},
        },
    },
    ("KR0075", "2025.1Q"): {
        ...
        "findings": {
            "3_tier2_composition": {"flag": "COMPOSITION_NEITHER", "residual": -243.09},
            "47_tier2_census": {"flag": "TIER2_DUPLICATE_ROW", "residual": None},
            "47_tier2_census_post": {"flag": "TIER2_DUPLICATE_ROW", "residual": None},
            "51_tfi_tier2_composition": {"flag": "TFI_COMPOSITION_NEITHER", "residual": -243.31},
        },
    },
    # --- 동양생명 KR0087 2025.2Q ---------------------------------------------
    ...
    # --- 한화생명 KR0068 2025.2Q ----------------------------------------------
    # **이 버킷만 사유가 다르다. 인과가 규명되지 않았다.** 위 버킷들은 "발행사가 같은 개념을
    # 두 값으로 인쇄했다" 를 산수로 증명하지만, 여기는 **잔차가 실재하는데 원문 어디에도 그
    # 항목이 없다.** 그래서 원장 status 도 `VERIFIED_BY_OWNER` 로 따로 갈랐다(게이트가 매 실행
    # `EXEMPTION_STANDS_ON_OWNER_JUDGEMENT` review 로 인쇄한다 — 조용해지지 않는다).
    #
    # **owner 가 2026-08-24 raw PDF 를 직접 열어 보고 결정했다**: "차이가 있는 건 사실인데
    # 원문이 그렇게 적혀 있고 별다른 언급은 없다. 원문대로 오차 용인하고 처리하자."
    #
    # raw FY2025_Q2 p17 (헤드라인, 억원) 직접 재확인(validation 독립 판독):
    #   지급여력금액 221,809 = 기본자본 82,506 + 보완자본 139,303        <- 헤드라인은 닫힌다
    #   Ⅰ 순자산 213,475 · Ⅱ 불인정 30,921 · Ⅲ 재분류 100,874
    #   주2) "기본자본은 … 순자산에서 지급여력금액 불인정 항목(단, **보완자본 한도를 초과한
    #        금액을 제외**) 및 보완자본으로 재분류하는 항목을 차감한 금액"
    #
    # **잔차가 두 개로 읽힌다 — 각주의 괄호(한도초과 제외)를 어떻게 읽느냐에 달렸다.**
    #   ① 한도초과 = 0 (괄호 무시):  213,475 − 30,921 − 100,874 = 81,680  vs 82,506 -> **+826**
    #   ② 한도초과 = min(item47−item48, item12) = min(70,821.29, 30,921) = 30,921 (룰의 읽기):
    #       213,475 − (30,921 − 30,921) − 100,874 = 112,601             vs 82,506 -> **−30,095**
    # **owner 에게 제시된 숫자는 ①의 826 이지만, 룰이 실제로 내는 RED 의 잔차는 ②의 −30,095**
    # 이다(branch=CAPPED). 박제는 **룰이 내는 값**이어야 재검산이 성립하므로 −30,095.00 을
    # 박고, 826 은 아래 note 와 원장에 같이 남긴다. 826 을 박으면 등재 즉시
    # `TIER2_EXEMPTION_RESIDUAL_DRIFT` 로 RED 가 되어 면제가 성립하지 않는다(실측 확인).
    # 두 잔차는 같은 축의 같은 불일치를 서로 다른 각주 해석으로 잰 값이다.
    #
    # **미규명 단서(설명이 아니다)**: item51 후 140,128.28 − 전 139,302.53 = **825.75** 로
    # ①의 필요 잔차 826.00 과 반올림 오차 이내다. **우연일 수 있고 owner 도 원인을 못 찾았다** —
    # 사유로 쓰지 않는다. 그 관계가 규명되면 등재를 재검토한다. 후속 티켓
    # `inbox/validation/20260824T0410Z` 는 **열어 둔다** — 면제는 push 를 푼 것이지 원인을
    # 닫은 것이 아니다.
    #
    # **해제조건**: 다리 입력 4칸(item2·4·12·13)을 전부 박았으므로 하나라도 움직이면
    # `TIER2_EXEMPTION_INPUT_DRIFT` RED, 결측이면 `..._INPUT_MISSING` RED 다. 갈래를 정하는
    # item47·48 과 단서 item51(전·후)도 같이 박았다 — 갈래가 CAPPED 에서 벗어나면 flag 대조가
    # `TIER2_EXEMPTION_RESIDUAL_DRIFT` 로 잡는다.
    ...
    ("KR0068", "2025.2Q"): {
        "cells": {1: {"값": 221809.0}, 2: {"값": 82506.0}, 3: {"값": 139303.0},
                  4: {"값": 213475.0}, 12: {"값": 30921.0}, 13: {"값": 100874.0},
                  47: {"값": 140128.28, "값_적용후": 140128.28},
                  48: {"값": 69306.99, "값_적용후": 69306.99},
                  49: {"값": 69995.55, "값_적용후": 69995.55},
                  50: {"값": 82506.15, "값_적용후": 82506.15},
                  51: {"값": 139302.53, "값_적용후": 140128.28},
                  52: {"값": 221808.68, "값_적용후": 222634.43}},
        "findings": {
            "2_tier1_bridge": {"flag": "branch=CAPPED", "residual": -30095.0},
        },
    },
}
