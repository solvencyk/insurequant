export const meta = {
  name: 'recover-market-subrisks',
  description: 'Read localized K-ICS market-risk pages, extract 시장위험 sub-risks 36-40 (+IRR 41-46), sqrt-reconcile gate, adversarial re-read on miss',
  phases: [
    { title: 'Extract', detail: 'one agent per (co,quarter) reads localized pages' },
...
function extractPrompt(it, feedback) {
  return [
    ...
    `Read this file with the Read tool — it is the localized market-risk section pages (text + table rows) of the disclosure PDF:`,
    `  ${it.file}`,
    ``,
    `Extract these 5 sub-risk amounts FOR THE CURRENT PERIOD (${it.quarter}). If 경과조치 적용전/적용후 both appear use 적용후. If 표준모형 vs 내부모형(internal model) both appear use 표준(standard) K-ICS. All values are in 백만원 (million KRW) EXACTLY AS PRINTED — do not rescale, do not strip digits:`,
    ...
    `Smaller P&C filings instead have ONE table with rows 금리위험/주식위험/부동산위험/외환위험/자산집중위험 — use those.`,
    ``,
...
// args = compact [[code, quarter, item19_eok, name], ...]; reconstruct file path + even
const rawArgs = typeof args === 'string' ? JSON.parse(args) : (args || [])
const items = rawArgs.map(([code, quarter, item19_eok, name]) => ({
  code, quarter, item19_eok, name,
  file: `artifacts/kics_validation/market_pages/${code}_${quarter}.md`,
...
const results = await pipeline(items,
  ...
  (it) => agent(extractPrompt(it), { label: `mkt:${it.code}_${it.quarter}`, phase: 'Extract', schema: SUB_SCHEMA })
            .then(r => ({ it, r })),
  // stage 2: reconcile; adversarial re-read once on near-miss; classify
  async ({ it, r }) => {
    if (!r) return { ...it, status: 'NULL' }
    let rc = reconcile(r.found, it.item19_eok)
    let used = r
    // re-read if we have signal (>=2 subs) but it doesn't reconcile, or 3 of 5 present
    ...
    if (iv.every(x => x !== null)) {
      const der = deriveIrr(iv[0],iv[1],iv[2],iv[3],iv[4],iv[5])  // 백만원 -> derived 금리위험액
