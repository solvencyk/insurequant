...
def main(argv=None) -> int:
    ...
    fast = ["tests/test_kics_rules_golden.py", "tests/test_master_tables_golden.py",
            ...
            # 항등식 레지스트리(2026-08-25 신설, owner "당연히 1 이어야돼"). **등식이어야 할
            # 것이 밴드로 구현되는 것**을 막는다 — `_XCHK_LO/_HI = 0.4, 2.5` 가 아무 선언 없이
            # 태어나 몇 달을 살면서 346버킷 중 0건을 잡던 상태가 이 테스트가 생긴 이유다.
            # 여기 안 넣으면 레지스트리도 honor-system 이 된다(이 훅이 생긴 이유 그 자체).
            # 변이시험 3종(선언삭제·밴드확대·룰무력화)이 실제로 발화하는 것을 확인했다. <1초.
            ...
            # 분기 지평 트립와이어(2026-08-29 신설). 게이트 세 곳이 각자 분기 목록을 리터럴로
            # 들고 있었고 셋 다 2026.1Q 에서 끝나 있어서, 2026.2Q 를 배포한 날 census·qoq·
            # spike·wfy·continuity 가 **그 분기를 순회조차 안 했다**(RED=0 = "안 봤다").
            # 사각에 `흥국화재 2026.2Q` PL 5항목 결측이 숨어 있었다. 지평을 마스터에서
            # 파생하도록 바꿨고, 이 테스트가 ① 파생 지평이 마스터 최신 분기를 품는지
            # ② 게이트가 리터럴 지평을 다시 심지 않았는지를 강제한다. 변이시험 확인:
            # QS 를 옛 리터럴로 되돌리면 2건 FAIL. <1초.
            "tests/test_quarter_horizon.py",
            # jp 레인 출처 게이트(2026-09-13 신설, UH-18). census 의 posted 행이 만료 호스트를
            # 1차 출처로 쓰는지 · 죽은 URL 인지 · **점검을 돌리긴 했는지**(증거 신선도)를
            # `build_jesr_page_json.py` self-check 가 막는다. 변이시험 5종(배선 끊기·만료호스트
            # 집합 비우기·dead 분류 비우기·절차룰 면제 허용·예외 필수키 검사 제거)이 전부
            # 발화하는 것을 확인했다. 여기 안 넣으면 "빌더를 안 돌리면 게이트도 안 돈다" 라
            # 또 honor-system 이 된다. <1초.
            "tests/test_jp_source_gate.py",
            # 불변식 1번의 jp 판(2026-09-13 신설, UH-19). **배포 JSON = census 재빌드 결과**.
            # census 만 고치고 jp/jesr_esr.json 을 재생성 안 한 커밋이 실제로 있었고(62eed63),
