#!/usr/bin/env python3
"""Download FY2026.2Q IR materials for 13 sources covering multiple K-ICS entities.

Copied from download_ir_2026q1.py per DL-FYR (2026.2Q+: reuse existing configs, swap
only period-specific labels). Most XPaths are positional (tr[1]/li[1]-style "latest
item") and need no change; KR0011_factsheet's URL embeds a Q1-dated filename and
KR1000's url2 embeds an intSeq that's known to vary per quarter -- both re-verified
live for 2026.2Q (see inline comments). KR0010/KR0032/KR0087 already have 2026.2Q raw
from an earlier ad-hoc fetch (`_groups/`, `KR0087_동양생명/`) -- kept in SOURCES anyway
for a fresh/complete sweep per owner request, harmless if redundant.

Output: data/ir/FY2026_Q2/<KR>_<name>/<filename>
Manifest: data/ir/FY2026_Q2/_manifest.json
"""
...
OUT_BASE = ROOT / "data" / "ir" / "FY2026_Q2"
...
SOURCES = {
    ...
    "KR0069": {  # 삼성생명 -- Vue/React SPA (sslife_v1), 3s wasn't enough for client routing
        # to finish before the goto/click sequence (2026-08-15 diagnosis: failure dump showed
        # only the "doesn't work without JavaScript" pre-load shell, not the real content).
...
def main() -> int:
    ...
    # Merge into any existing manifest by key -- a partial rerun (e.g. retrying just the
    # failures) must not blank out the other sources' already-recorded results (bit us
    # 2026-08-17: a 2-key rerun clobbered the full 13-source manifest down to 2 entries).
    manifest_path = OUT_BASE / "_manifest.json"
    prior_by_key: dict[str, dict] = {}
    if manifest_path.exists():
        try:
            prior = json.loads(manifest_path.read_text(encoding="utf-8"))
            prior_by_key = {r["key"]: r for r in prior.get("results", [])}
        except Exception:
            pass

    with sync_playwright() as p:
        for k, cfg in targets.items():
            prior_by_key[k] = _run_one(p, k, cfg)
