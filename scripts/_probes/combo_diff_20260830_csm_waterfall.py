...
build_csm() only). Reports: row-count delta, cell-count delta (any row/field
appearing/disappearing), and a full list of every (code, quarter, item) whose
'값' or '값_당분기' changed, grouped by company so an unexpected non-KR0079
company jumps out immediately.
"""
import sys, json
...
before = json.loads((ROOT / "CSM_waterfall.json.bak_20260830_kr0079wideprod").read_text(encoding="utf-8"))
...
out = ROOT / "scripts/_probes/_out_20260830_combo_diff_csm_waterfall.txt"
