# -*- coding: utf-8 -*-
"""Scoped rebuild for inbox/parser/20260901T1630Z (pl_gap14): KR0072(KDB생명) 2023.1Q
_GOLD_CELL_OVERRIDE 신규 + KR0150(서울보증) _sgi_re_legs 라벨변형 수정.

Mirrors scripts/_probes/probe_20260901_pl_csm_amort_scoped_build.py's pattern: re-parse
ONLY these 2 companies via discover_filings/parse_filing/assemble and UNION-MERGE the
resulting rows into data/dart/viz/pl_breakdown_master.json + pl_breakdown_coverage.json by
company key -- every other company's rows pass through untouched. This is the company-
scoped call CLAUDE.md requires instead of build_pl_breakdown.main() (full 39-company
rebuild, higher blast radius than needed for a 2-company code fix)."""
...
TARGET_CODES = {"KR1010", "KR0003", "KR0008", "KR0032"}
    ...
    for code in sorted(c for c in filings if c in TARGET_CODES):
        ...
        for q in sorted(parsed, key=_quarter_sort_key):
            ...
            if t1 is None and t2 is None and not has_override:
                ...
                print(f"  SKIP {code} {q}: t1=None t2=None (status={st}) -- override NOT applied")
                continue
            ...
            if ov:
                for _k, _val in ov.items():
                    v[_k] = _val
                v["_reconciled"] = True
                print(f"  OVERRIDE applied {code} {q}: {sorted(ov)}")
            for n in range(1, 25):
                rows.append({
