...
for f in files:
    ...
    if selected is None:
        continue
    t, header = selected
    header_len_dist[len(header)] += 1
    if len(header) != 3:
        odd_ones.append((f.relative_to(MD_INBOX).as_posix(), header))
print("header length distribution:", dict(sorted(header_len_dist.items())))
print()
for name, header in odd_ones:
    print(name, header)
