# -*- coding: utf-8 -*-
"""Batch A4 reinsurance rollforward on cached annual filings (23 operational insurers)."""
...
SUMMARY_OUT = settings.extracted_dir / "_batch_reinsurance_summary.json"
...
def main():
    ...
    for i, row in enumerate(targets, 1):
        ...
        for xml in sorted(raw_dir.glob("*.xml")):
            try:
                tables = extract_reinsurance_tables(xml, company_name=kics)
        ...
        out_json = settings.extracted_dir / f"{canonical}_{rcept_no}_reinsurance.json"
        out_json.write_text(json.dumps(all_tables, ensure_ascii=False, indent=2), encoding="utf-8")
        mvp_json = settings.extracted_dir / f"{canonical}_{rcept_no}_reinsurance_mvp.json"
