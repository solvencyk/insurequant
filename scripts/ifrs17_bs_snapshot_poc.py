# -*- coding: utf-8 -*-
"""Skim 5 PoC companies annual filings for B1 BS snapshot rollforward."""
...
def main():
    ...
    for dirname, company_name in POC_DIRS:
        ...
        for xml in sorted(d.glob("*.xml")):
            tables = extract_bs_snapshot_tables(xml, company_name=company_name)
            per_xml.append({
                "xml": xml.name,
                "tables": len(tables),
        ...
        out = settings.extracted_dir / f"{dirname}_bs_snapshot.json"
    ...
    out_summary = settings.extracted_dir / "_bs_snapshot_poc_summary.json"
