...
PL_CONSTRUCTIVE_GUARDED = {
    ...
    23: "법인세 — **좁은 커버리지**: assemble 이 22-24 로 무조건 덮어쓰는 건 여전하지만, "
        "PL_YTD_COLLAPSE_TO_ZERO(같은 FY 인접분기, 직전값 non-zero -> 이번값 정확히 0.0) 가 "
        "'덮어쓴 값이 우연히 0.0' 인 경우만 잡는다 — item23 오염 전반을 보는 룰이 아니다. "
        "2026-09-12(inbox/parser/20260901T1630Z) 실측: KR0072(케이디비생명) 2023.1Q 를 신규로 "
        "채우자(item23=-0.209323, 원문 직접값) 그 전엔 존재하지 않았던 '직전분기' 가 생겼고, "
        "변이시험이 2023.2Q(기존 override 값 -0.2)를 22-24 로 재계산하면 정확히 0.0 이 되어 "
        "걸렸다(대상 회사-분기가 한 번도 없었으면 이 축은 여전히 BLIND — 회사별 인접분기 "
        "존재 여부에 좌우된다는 점에서 22 번(2f, 전항 무조건 적용)과 다르다).",
}
...
GOLD_OVERLAY_CENSUS = {"CSM": (276, 86), "PL": (203, 29)}
