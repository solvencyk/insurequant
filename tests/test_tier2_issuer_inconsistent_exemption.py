...
def test_the_exemption_is_narrow_and_does_not_touch_the_held_buckets(records, findings):
    ...
    **두 버킷의 기대가 다르다는 점이 중요하다.** 둘 다 '레지스트리에 절대 없어야 한다' 는
    ...
    # 2026-09-01: 18 -> 19. 농협생명 KR0104 2026.2Q (`47_tier2_census_post` /
    # `TIER2_DUPLICATE_ROW`) 1버킷 추가. 발주 inbox/validation/20260831T152646Z__parser__
    # KR0104_2026.2Q__tier2_duplicate_row_exception.md. 근거는 **MD 가 아니라 raw PDF** 를
    # 직접 열어 확인했다 — data/disclosure/FY2026_Q2/pdf/KR0104_농협생명보험.pdf p20 에서
    # '보완자본 한도 적용 전'(1,442,557 / 1,192,557) 과 '보완자본 한도'(1,719,757 / 1,192,557)
    # 가 word 좌표로 분리된 **두 개의 행**인데 적용후 컬럼에서만 같은 값을 인쇄한다. 더해서
    # 적용후 한도 1,192,557 은 SCR 3,439,515 × 50% = 1,719,757.5 와도 어긋난다(발행사 자기모순).
