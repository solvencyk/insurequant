...
def build_one_period(
    ...
    # Priority new-business path (same as build_for_file): the 전환방법별
    # 보험계약마진 변동 single-cell (신계약 인식효과 / 합계) overrides the leaf-col
    # new_business when the filing has that table. Computed on raw (unscaled)
    # blocks so it must be injected BEFORE _scale_stages divides the stages.
    nb_override = transition_new_business(blocks)

    def _scale_stages(stages: dict) -> float:
        if nb_override is not None:
            stages["new_business"] = dict(nb_override)
    ...
    # Reject candidates whose new-business CSM is negative: cumulative NB CSM can
    # never be < 0. A negative means find_csm_leaf_cols mis-identified the CSM
    # column as BEL — observed in 2025 반기/3분기 deep multi-level-header tables
    # whose CSM is split by 전환방법 (수정소급/공정가치/그외) with empty leading
    # label cells, so the fallback returns [0]=BEL. Emitting an honest gap beats a
    # garbage negative (DB손해 2025.2Q −16,414 / 미래에셋 등). Positive / unmatched
    # (None) candidates are kept, so valid quarters are untouched.
    _nn = [
        b for b in ranked
        if ((extract_stages(b).get("new_business") or {}).get("value_mn_krw") or 0) >= 0
    ]
    ranked = _nn
    if not ranked:
