# -*- coding: utf-8 -*-
"""Batch A3 insurance P&L rollforward on cached annual filings (23 operational insurers)."""
...
from src.ifrs17.insurance_pl_extractor import extract_insurance_pl_tables, to_jsonable
from src.ifrs17.universe import is_excluded

SUMMARY_IN = settings.extracted_dir / "_batch_all_summary.json"
SUMMARY_OUT = settings.extracted_dir / "_batch_insurance_pl_summary.json"
...
def main():
    ...
    for i, row in enumerate(targets, 1):
        ...
        for xml in sorted(raw_dir.glob("*.xml")):
            try:
                tables = extract_insurance_pl_tables(xml, company_name=kics)
        ...
        out_json = settings.extracted_dir / f"{canonical}_{rcept_no}_insurance_pl.json"
        out_json.write_text(json.dumps(all_tables, ensure_ascii=False, indent=2), encoding="utf-8")
        mvp_json = settings.extracted_dir / f"{canonical}_{rcept_no}_insurance_pl_mvp.json"
